...
import io.nightona.toolbox.client.ApiClient;
import io.nightona.toolbox.client.api.FileSystemApi;
import io.nightona.toolbox.client.model.FilesDownloadRequest;
import okhttp3.Call;
...
final class FileTransfer {
    ...
    static InputStream streamDownload(FileSystemApi fileSystemApi, String remotePath, DownloadStreamOptions options) throws io.nightona.sdk.exception.NightonaException {
        int timeoutSeconds = options.getTimeoutSeconds();
        if (timeoutSeconds < 0) {
            throw new io.nightona.sdk.exception.NightonaException("Timeout must be non-negative");
        }

        ApiClient apiClient = fileSystemApi.getApiClient();
        if (apiClient == null || apiClient.getBasePath() == null || apiClient.getBasePath().isEmpty()) {
            throw new io.nightona.sdk.exception.NightonaException("Toolbox client is not configured");
        }
        ...
        try {
            ...
            if (call.isCanceled()) {
                throw new io.nightona.sdk.exception.NightonaException("Download cancelled");
            }
            throw new io.nightona.sdk.exception.NightonaException("Failed to download file stream", e);
        } finally {
            // On any failure path (IOException *or* RuntimeException / NightonaException
    ...
    private static InputStream extractDownloadFileStream(Response response, DownloadStreamOptions options, Call call) throws IOException {
        ...
        if (responseBody == null) {
            response.close();
            throw new io.nightona.sdk.exception.NightonaException("Download response body is empty");
        }
        ...
        if (boundary == null || boundary.isEmpty()) {
            response.close();
            throw new io.nightona.sdk.exception.NightonaException("Missing multipart boundary in download response");
        }
        ...
        try {
            ...
            response.close();
            throw new io.nightona.sdk.exception.NightonaException("File stream not found in download response");
        } catch (IOException e) {
            response.close();
            if (call.isCanceled()) {
                throw new io.nightona.sdk.exception.NightonaException("Download cancelled");
    ...
    private static Map<String, String> readPartHeaders(BufferedInputStream inputStream) throws IOException {
        ...
        while (true) {
            String line = readLine(inputStream);
            if (line == null) {
                throw new io.nightona.sdk.exception.NightonaException("Unexpected end of multipart response");
            }
            if (line.isEmpty()) {
                return headers;
            }
    ...
    private static io.nightona.sdk.exception.NightonaException parseDownloadError(byte[] body, int fallbackStatusCode) {
    ...
    private static final class MultipartPartInputStream extends InputStream {
        ...
        private boolean fill() throws IOException {
            ...
            try {
                ...
                if (call.isCanceled()) {
                    throw new io.nightona.sdk.exception.NightonaException(cancelledMessage);
                }
                throw e;
            }
            if (read == -1) {
    ...
    static void streamUpload(FileSystemApi fileSystemApi, InputStream source, String remotePath, UploadStreamOptions options) {
        ...
        if (source == null) {
            throw new io.nightona.sdk.exception.NightonaException("Upload source must not be null");
        }
        if (remotePath == null || remotePath.isEmpty()) {
            throw new io.nightona.sdk.exception.NightonaException("remotePath must not be empty");
        }
        ...
        try (Response response = call.execute()) {
            ...
            if (call.isCanceled()) {
                throw new io.nightona.sdk.exception.NightonaException("Upload cancelled");
            }
            throw new io.nightona.sdk.exception.NightonaException("Failed to upload file stream", e);
        } finally {
            unregister.run();
        }
    }
