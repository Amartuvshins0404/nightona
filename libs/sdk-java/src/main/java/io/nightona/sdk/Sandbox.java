...
import io.nightona.api.client.model.BuildInfo;
...
import io.nightona.api.client.model.SandboxListItem;
...
public class Sandbox {
    ...
    private final io.nightona.toolbox.client.ApiClient toolboxApiClient;
    private final io.nightona.toolbox.client.api.InfoApi infoApi;
    private final String apiKey;

    // Fields shared by both io.nightona.api.client.model.Sandbox and SandboxListItem.
    private String id;
    private String name;
    ...
    // Fields only present on the full Sandbox DTO; not populated by Nightona.list() —
    ...
    Sandbox(SandboxApi sandboxApi, NightonaConfig config, io.nightona.api.client.model.Sandbox data) {
        this.sandboxApi = sandboxApi;
        this.config = config;
        this.apiKey = config.getApiKey();
        populateFromDTO(data);
        this.toolboxApiClient = buildToolboxApiClient(sandboxApi, config);
        this.infoApi = new io.nightona.toolbox.client.api.InfoApi(toolboxApiClient);
        this.process = new Process(new io.nightona.toolbox.client.api.ProcessApi(toolboxApiClient), this);
        this.fs = new FileSystem(new io.nightona.toolbox.client.api.FileSystemApi(toolboxApiClient));
        this.git = new Git(new io.nightona.toolbox.client.api.GitApi(toolboxApiClient));
        this.computerUse = new ComputerUse(new io.nightona.toolbox.client.api.ComputerUseApi(toolboxApiClient));
        this.codeInterpreter = new CodeInterpreter(new io.nightona.toolbox.client.api.InterpreterApi(toolboxApiClient), this);
    }

    Sandbox(SandboxApi sandboxApi, NightonaConfig config, SandboxListItem data) {
        this.sandboxApi = sandboxApi;
        this.config = config;
        this.apiKey = config.getApiKey();
        populateFromDTO(data);
     ...
     */
    private io.nightona.toolbox.client.ApiClient buildToolboxApiClient(SandboxApi sandboxApi, NightonaConfig config) {
        ...
        io.nightona.toolbox.client.ApiClient client = new io.nightona.toolbox.client.ApiClient();
    ...
    public LspServer createLspServer(String languageId, String pathToProject) {
        return new LspServer(new io.nightona.toolbox.client.api.LspApi(toolboxApiClient));
    }

    String getLanguage() {
        String lang = "python";
        if (labels != null && labels.containsKey(Nightona.CODE_TOOLBOX_LANGUAGE_LABEL)) {
            lang = labels.get(Nightona.CODE_TOOLBOX_LANGUAGE_LABEL);
        }
        return lang;
    }
     ...
     *
     * @throws NightonaException if the Sandbox fails to start
     */
    public void start() {
        start(60);
    }
     ...
     * @throws NightonaException if start fails or times out
     */
    public void start(long timeoutSeconds) {
        io.nightona.api.client.model.Sandbox response = ExceptionMapper.callMain(() -> sandboxApi.startSandbox(id, null));
        if (response != null) {
     ...
     *
     * @throws NightonaException if the Sandbox fails to stop
     */
    public void stop() {
        stop(60);
    }
     ...
     *
     * @param timeoutSeconds maximum seconds to wait; {@code 0} disables timeout
     * @throws NightonaException if stop fails or times out
     */
     ...
     *
     * @param timeoutSeconds maximum seconds to wait; {@code 0} disables timeout
     * @throws NightonaException if timeout is invalid, state becomes error, or timeout expires
     */
    public void waitUntilStopped(long timeoutSeconds) {
        if (timeoutSeconds < 0) {
            throw new NightonaException("Timeout must be non-negative");
        }
        ...
        while (!"stopped".equalsIgnoreCase(state) && !"destroyed".equalsIgnoreCase(state)) {
            ...
            if ("error".equalsIgnoreCase(state)) {
                throw new NightonaException("Sandbox entered error state while stopping");
            }
            if (timeoutSeconds > 0 && (System.currentTimeMillis() - startedAt) > timeoutSeconds * 1000L) {
                throw new NightonaException("Sandbox failed to stop before timeout");
            }
            try {
                Thread.sleep(250);
            } catch (InterruptedException e) {
                Thread.currentThread().interrupt();
                throw new NightonaException("Interrupted while waiting for sandbox stop", e);
            }
        }
    }
     ...
     *
     * @throws NightonaException if deletion fails
     */
    public void delete() {
        delete(60);
    }
     ...
     * @throws NightonaException if label update fails
     */
     ...
     * @throws NightonaException if the update fails
     */
    public void setAutostopInterval(int minutes) {
        io.nightona.api.client.model.Sandbox response = ExceptionMapper.callMain(() -> sandboxApi.setAutostopInterval(id, BigDecimal.valueOf(minutes), null));
        if (response != null) {
            populateFromDTO(response);
        }
    }
    ...
    public void setAutoArchiveInterval(int minutes) {
        io.nightona.api.client.model.Sandbox response = ExceptionMapper.callMain(() -> sandboxApi.setAutoArchiveInterval(id, BigDecimal.valueOf(minutes), null));
        if (response != null) {
            populateFromDTO(response);
        }
    }
    ...
    public void setAutoDeleteInterval(int minutes) {
        io.nightona.api.client.model.Sandbox response = ExceptionMapper.callMain(() -> sandboxApi.setAutoDeleteInterval(id, BigDecimal.valueOf(minutes), null));
        if (response != null) {
            populateFromDTO(response);
        }
    }
    ...
    public void updateNetworkSettings(UpdateSandboxNetworkSettings settings) {
        io.nightona.api.client.model.Sandbox response = ExceptionMapper.callMain(() -> sandboxApi.updateNetworkSettings(id, settings, null));
        if (response != null) {
            populateFromDTO(response);
        }
    }
     ...
     * @throws NightonaException if the request fails
     */
    public String getUserHomeDir() {
        io.nightona.toolbox.client.model.UserHomeDirResponse value = ExceptionMapper.callToolbox(() -> infoApi.getUserHomeDir());
    ...
    public String getWorkDir() {
        io.nightona.toolbox.client.model.WorkDirResponse value = ExceptionMapper.callToolbox(() -> infoApi.getWorkDir());
        return value == null ? "" : asString(value.getDir());
    }
     ...
     *
     * @param timeoutSeconds maximum seconds to wait; {@code 0} disables timeout
     * @throws NightonaException if timeout is invalid, state becomes failure, or timeout expires
     */
    public void waitUntilStarted(long timeoutSeconds) {
        ...
        while (!"started".equalsIgnoreCase(state)) {
            ...
            if ("error".equalsIgnoreCase(state) || "build_failed".equalsIgnoreCase(state)) {
                throw new NightonaException("Sandbox entered failure state: " + state);
            }

            if (timeoutSeconds > 0 && (System.currentTimeMillis() - startedAt) > timeoutSeconds * 1000L) {
                throw new NightonaException("Sandbox failed to become started before timeout");
            }
            ...
            try {
                ...
                Thread.currentThread().interrupt();
                throw new NightonaException("Interrupted while waiting for sandbox start", e);
            }
        }
    }
     ...
     * — including those not returned by {@link Nightona#list} — are populated.
     *
     * @throws NightonaException if refresh fails
     */
    public void refreshData() {
        io.nightona.api.client.model.Sandbox data = ExceptionMapper.callMain(() -> sandboxApi.getSandbox(id, null, null));
        if (data != null) {
            populateFromDTO(data);
        }
    }
    ...
    /**
     * Copies fields from the full {@link io.nightona.api.client.model.Sandbox} DTO onto this instance.
     *
     ...
     */
    private void populateFromDTO(io.nightona.api.client.model.Sandbox d) {
        if (d == null) {
            return;
        }
     ...
     * @throws NightonaException if the fork operation fails or times out
     */
    ...
    public Sandbox experimentalFork(String name, long timeoutSeconds) {
        ...
        io.nightona.api.client.model.Sandbox response = ExceptionMapper.callMain(
     ...
     * @throws NightonaException if the snapshot operation fails
     */
    ...
    private void waitForSnapshotComplete(long timeoutSeconds) {
        ...
        while ("snapshotting".equalsIgnoreCase(state)) {
            refreshData();
            if ("error".equalsIgnoreCase(state) || "build_failed".equalsIgnoreCase(state)) {
                throw new NightonaException("Sandbox snapshot failed with state: " + state);
            }
            if (!"snapshotting".equalsIgnoreCase(state)) {
                return;
            }
            if (timeoutSeconds > 0 && (System.currentTimeMillis() - startedAt) > timeoutSeconds * 1000L) {
                throw new NightonaException("Sandbox snapshot did not complete before timeout");
            }
            try {
                Thread.sleep(250);
            } catch (InterruptedException e) {
                Thread.currentThread().interrupt();
                throw new NightonaException("Interrupted while waiting for snapshot complete", e);
            }
        }
    }
     ...
     * Uses default timeout of 60 seconds.
     *
     * @throws NightonaException if the pause operation fails
     */
    public void pause() throws NightonaException {
        pause(60);
    }
     ...
     * @throws NightonaException if timeout is negative or the operation fails/times out
     */
    public void pause(long timeoutSeconds) throws NightonaException {
        if (timeoutSeconds < 0) {
            throw new NightonaException("Timeout must be a non-negative number");
        }
    ...
    private void waitForPauseComplete(long timeoutSeconds) {
        ...
        while ("pausing".equalsIgnoreCase(state)) {
            refreshData();
            if ("error".equalsIgnoreCase(state) || "build_failed".equalsIgnoreCase(state)) {
                throw new NightonaException("Sandbox pause failed with state: " + state);
            }
            if (!"pausing".equalsIgnoreCase(state)) {
                return;
            }
            if (timeoutSeconds > 0 && (System.currentTimeMillis() - startedAt) > timeoutSeconds * 1000L) {
                throw new NightonaException("Sandbox pause did not complete before timeout");
            }
            try {
                Thread.sleep(250);
            } catch (InterruptedException e) {
                Thread.currentThread().interrupt();
                throw new NightonaException("Interrupted while waiting for pause complete", e);
            }
        }
    }
    ...
    /** @return Nightona snapshot used to create this Sandbox, or {@code null} if none. */
     ...
     *
     * <p>Not returned by {@link Nightona#list}; call {@link #refreshData()} on each item to populate.
     *
    ...
    io.nightona.toolbox.client.ApiClient getToolboxApiClient() { return toolboxApiClient; }
