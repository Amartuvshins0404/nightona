...
import io.nightona.sdk.model.*;
import io.nightona.sdk.exception.NightonaException;
import io.nightona.toolbox.client.api.ProcessApi;
import io.nightona.toolbox.client.model.CodeRunRequest;
import io.nightona.toolbox.client.model.CodeRunResponse;
import io.nightona.toolbox.client.model.CreateSessionRequest;
import io.nightona.toolbox.client.model.ExecuteRequest;
import io.nightona.toolbox.client.model.PtyCreateRequest;
import io.nightona.toolbox.client.model.PtyCreateResponse;
import io.nightona.toolbox.client.model.PtyListResponse;
import io.nightona.toolbox.client.model.PtyResizeRequest;
import io.nightona.toolbox.client.model.PtySessionInfo;
import io.nightona.toolbox.client.model.SessionSendInputRequest;
...
public class Process {
     ...
     * @throws NightonaException if execution fails
     */
    ...
    public ExecuteResponse executeCommand(String command, String cwd, Map<String, String> env, Integer timeout) {
        ...
        io.nightona.toolbox.client.model.ExecuteResponse response = ExceptionMapper.callToolbox(() -> processApi.executeCommand(request));
     ...
     * @throws NightonaException if session creation fails
     */
     ...
     * @throws NightonaException if retrieval fails
     */
    public Session getSession(String sessionId) {
        io.nightona.toolbox.client.model.Session session = ExceptionMapper.callToolbox(() -> processApi.getSession(sessionId));
    ...
    public Session getEntrypointSession() {
        io.nightona.toolbox.client.model.Session session = ExceptionMapper.callToolbox(processApi::getEntrypointSession);
        return toSession(session);
    }
    ...
    public SessionExecuteResponse executeSessionCommand(String sessionId, SessionExecuteRequest req) {
        io.nightona.toolbox.client.model.SessionExecuteRequest request = new io.nightona.toolbox.client.model.SessionExecuteRequest()
                .command(req.getCommand())
                .runAsync(req.getRunAsync());
        io.nightona.toolbox.client.model.SessionExecuteResponse response = ExceptionMapper.callToolbox(() -> processApi.sessionExecuteCommand(sessionId, request));
    ...
    public Command getSessionCommand(String sessionId, String commandId) {
        io.nightona.toolbox.client.model.Command command = ExceptionMapper.callToolbox(() -> processApi.getSessionCommand(sessionId, commandId));
     ...
     * @throws NightonaException if streaming fails
     */
     ...
     * @throws NightonaException if sending input fails
     */
     ...
     *
     * @return session list
     * @throws NightonaException if listing fails
     */
    public List<Session> listSessions() {
        List<io.nightona.toolbox.client.model.Session> sessions = ExceptionMapper.callToolbox(processApi::listSessions);
        List<Session> output = new ArrayList<Session>();
        if (sessions != null) {
            for (io.nightona.toolbox.client.model.Session session : sessions) {
     ...
     * @throws NightonaException if PTY session creation fails
     */
    public PtyHandle createPty(PtyCreateOptions options) {
        ...
        if (response == null || response.getSessionId() == null || response.getSessionId().isEmpty()) {
            throw new NightonaException("Failed to create PTY session");
        }
     ...
     * @return PTY handle for streaming I/O and lifecycle operations
     * @throws NightonaException if websocket connection setup fails
     */
     ...
     * @throws NightonaException if resize fails
     */
     ...
     *
     * @param sessionId PTY session identifier
     * @throws NightonaException if termination fails
     */
    ...
    private String buildPtyWebSocketUrl(String toolboxBaseUrl, String sessionId) {
        if (toolboxBaseUrl == null || toolboxBaseUrl.isEmpty()) {
            throw new NightonaException("Toolbox base URL is not available");
        }
    ...
    private void streamDemuxedLogsViaWebSocket(String wsUrl, Consumer<String> onStdout, Consumer<String> onStderr) {
        ...
        sandbox.getToolboxApiClient().getHttpClient().newWebSocket(wsRequest, new WebSocketListener() {
            ...
            public void onFailure(WebSocket webSocket, Throwable t, Response response) {
                ...
                failure.compareAndSet(null, new NightonaException("Log streaming failed: " + msg, t));
                doneLatch.countDown();
            }
        ...
        try {
            ...
            throw new NightonaException("Interrupted while streaming logs", e);
        }
    ...
    private ExecuteResponse toExecuteResponse(io.nightona.toolbox.client.model.ExecuteResponse source) {
    ...
    private Session toSession(io.nightona.toolbox.client.model.Session source) {
        return new Session(source);
    }

    private SessionExecuteResponse toSessionExecuteResponse(io.nightona.toolbox.client.model.SessionExecuteResponse source) {
