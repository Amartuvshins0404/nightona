...
public class PtyHandle {
     ...
     * @throws NightonaException if connection fails or times out
     */
    public void waitForConnection(long timeoutSeconds) {
        ...
        try {
            ...
            if (!ready) {
                throw new NightonaException("Timed out waiting for PTY connection");
            }
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
            throw new NightonaException("Interrupted while waiting for PTY connection", e);
        }

        if (error != null && !error.isEmpty()) {
            throw new NightonaException("PTY connection failed: " + error, failure);
        }
        if (!connectionEstablished) {
            throw new NightonaException("PTY connection was not established");
        }
    }
     ...
     * @throws NightonaException if sending fails
     */
    public void sendInput(String data) {
        if (!ws.send(data)) {
            throw new NightonaException("Failed to send PTY input");
        }
    }
    ...
    public void sendInput(byte[] data) {
        if (!ws.send(ByteString.of(data))) {
            throw new NightonaException("Failed to send PTY binary input");
        }
    }
     ...
     * @throws NightonaException if interrupted while waiting
     */
    public PtyResult waitForExit() {
        try {
            ...
            Thread.currentThread().interrupt();
            throw new NightonaException("Interrupted while waiting for PTY exit", e);
        }
    }
