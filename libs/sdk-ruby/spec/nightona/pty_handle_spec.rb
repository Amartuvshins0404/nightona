...
RSpec.describe Nightona::PtyHandle do
  ...
  describe Nightona::PtyResult do
  ...
  describe '#wait_for_connection' do
    ...
    it 'raises when the timeout is exceeded' do
      ...
      expect { handle.wait_for_connection(timeout: 0) }
        .to raise_error(Nightona::Sdk::Error, /PTY connection timeout/)
    end
  end
  ...
  describe '#send_input' do
    ...
    it 'raises when the websocket is closed' do
      ...
      expect { handle.send_input('ls') }.to raise_error(Nightona::Sdk::Error, /not connected/)
    end
  end

  describe '#resize' do
    it 'delegates to the resize handler' do
      result = handle.resize(Nightona::PtySize.new(rows: 30, cols: 120))
    ...
    it 'raises when no resize handler is available' do
      ...
      expect { bare_handle.resize(Nightona::PtySize.new(rows: 10, cols: 20)) }
        .to raise_error(Nightona::Sdk::Error, /No resize handler available/)
    end
  end
  ...
  describe '#kill' do
    ...
    it 'raises when no kill handler is available' do
      ...
      expect { bare_handle.kill }.to raise_error(Nightona::Sdk::Error, /No kill handler available/)
    end
  end

  describe '#wait' do
    before do
      handle.instance_variable_set(:@status, Nightona::PtyHandle::Status::CONNECTED)
    end
  ...
  describe 'websocket event handling' do
    ...
    it 'closes websocket and raises on invalid control status' do
      ...
      end.to raise_error(Nightona::Sdk::Error, /Received invalid control message status: weird/)
