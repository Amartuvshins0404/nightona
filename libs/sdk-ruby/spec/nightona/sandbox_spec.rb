...
RSpec.describe Nightona::Sandbox do
  ...
  let(:process_api) { instance_double(NightonaToolboxApiClient::ProcessApi) }
  let(:fs_api) { instance_double(NightonaToolboxApiClient::FileSystemApi) }
  let(:git_api) { instance_double(NightonaToolboxApiClient::GitApi) }
  let(:lsp_api) { instance_double(NightonaToolboxApiClient::LspApi) }
  let(:computer_use_api) { instance_double(NightonaToolboxApiClient::ComputerUseApi) }
  let(:interpreter_api) { instance_double(NightonaToolboxApiClient::InterpreterApi) }
  let(:info_api) { instance_double(NightonaToolboxApiClient::InfoApi) }

  before do
    allow(NightonaToolboxApiClient::ApiClient).to receive(:new).and_return(toolbox_client)
    allow(NightonaToolboxApiClient::ProcessApi).to receive(:new).and_return(process_api)
    allow(NightonaToolboxApiClient::FileSystemApi).to receive(:new).and_return(fs_api)
    allow(NightonaToolboxApiClient::GitApi).to receive(:new).and_return(git_api)
    allow(NightonaToolboxApiClient::LspApi).to receive(:new).and_return(lsp_api)
    allow(NightonaToolboxApiClient::ComputerUseApi).to receive(:new).and_return(computer_use_api)
    allow(NightonaToolboxApiClient::InterpreterApi).to receive(:new).and_return(interpreter_api)
    allow(NightonaToolboxApiClient::InfoApi).to receive(:new).and_return(info_api)
  end
  ...
  describe '#initialize' do
    it 'populates attributes from DTO' do
      ...
      expect(sandbox.user).to eq('nightona')
    ...
    it 'creates process, fs, git, computer_use, and code_interpreter helpers' do
      expect(sandbox.process).to be_a(Nightona::Process)
      expect(sandbox.fs).to be_a(Nightona::FileSystem)
      expect(sandbox.git).to be_a(Nightona::Git)
      expect(sandbox.computer_use).to be_a(Nightona::ComputerUse)
      expect(sandbox.code_interpreter).to be_a(Nightona::CodeInterpreter)
    end
    ...
    it 'configures toolbox client authorization and sdk headers' do
      ...
      expect(toolbox_client.default_headers['X-Nightona-Source']).to eq('sdk-ruby')
      expect(toolbox_client.default_headers['X-Nightona-SDK-Version']).to eq(Nightona::Sdk::VERSION)
      expect(toolbox_client).to have_received(:user_agent=).with("sdk-ruby/#{Nightona::Sdk::VERSION}").at_least(:once)
    end

    it 'adds organization header when using JWT auth' do
      jwt_config = Nightona::Config.new(jwt_token: 'jwt', organization_id: 'org-9', api_url: 'https://api.example.com')

      described_class.new(sandbox_dto: sandbox_dto, config: jwt_config, sandbox_api: sandbox_api)

      expect(toolbox_client.default_headers['X-Nightona-Organization-ID']).to eq('org-9')
    end
  end
  ...
  describe '#auto_stop_interval=' do
    ...
    it 'raises on negative interval' do
      expect { sandbox.auto_stop_interval = -1 }
        .to raise_error(Nightona::Sdk::Error, /Auto-stop interval must be a non-negative integer/)
    end
  end
  ...
  describe '#auto_archive_interval=' do
    ...
    it 'raises on negative interval' do
      expect { sandbox.auto_archive_interval = -1 }
        .to raise_error(Nightona::Sdk::Error, /Auto-archive interval must be a non-negative integer/)
    end
  end
  ...
  describe '#create_ssh_access' do
    it 'delegates to sandbox_api' do
      ssh_dto = instance_double(NightonaApiClient::SshAccessDto)
  ...
  describe '#get_user_home_dir' do
    it 'returns the user home directory' do
      allow(info_api).to receive(:get_user_home_dir).and_return(double('DirResponse', dir: '/home/nightona'))

      expect(sandbox.get_user_home_dir).to eq('/home/nightona')
    end

    it 'wraps errors in Sdk::Error' do
      allow(info_api).to receive(:get_user_home_dir).and_raise(StandardError, 'connection refused')

      expect { sandbox.get_user_home_dir }.to raise_error(Nightona::Sdk::Error, /Failed to get user home directory/)
    end
  end
  ...
  describe '#get_work_dir' do
    ...
    it 'wraps errors in Sdk::Error' do
      ...
      expect { sandbox.get_work_dir }.to raise_error(Nightona::Sdk::Error, /Failed to get working directory path/)
    end
  end

  describe '#labels=' do
    it 'replaces labels via API' do
      label_request = instance_double(NightonaApiClient::SandboxLabels)
      label_response = instance_double(NightonaApiClient::SandboxLabels, labels: { 'env' => 'prod' })
      allow(NightonaApiClient::SandboxLabels).to receive(:build_from_hash).with(labels: { 'env' => 'prod' }).and_return(label_request)
  ...
  describe '#preview_url' do
    it 'delegates to sandbox_api.get_port_preview_url' do
      preview = instance_double(NightonaApiClient::PortPreviewUrl)
  ...
  describe '#create_signed_preview_url' do
    it 'delegates to sandbox_api with optional expiration' do
      signed_url = instance_double(NightonaApiClient::SignedPortPreviewUrl)
  ...
  describe '#refresh_activity' do
    ...
    it 'wraps errors in Sdk::Error' do
      ...
      expect { sandbox.refresh_activity }.to raise_error(Nightona::Sdk::Error, /Failed to refresh sandbox activity/)
    end
  end
  ...
  describe '#start' do
    it 'calls start_sandbox and waits for started state' do
      allow(sandbox_api).to receive(:start_sandbox).with('sandbox-123').and_return(build_sandbox_dto(state: NightonaApiClient::SandboxState::STARTED))
  ...
  describe '#recover' do
    it 'calls recover_sandbox and waits for started state' do
      allow(sandbox_api).to receive(:recover_sandbox).with('sandbox-123').and_return(build_sandbox_dto(state: NightonaApiClient::SandboxState::STARTED))
    ...
    it 'wraps errors in Sdk::Error' do
      ...
      expect { sandbox.recover(5) }.to raise_error(Nightona::Sdk::Error, /Failed to recover sandbox/)
    end
  end
  ...
  describe '#stop' do
    it 'calls stop_sandbox and waits for stopped state' do
      ...
      allow(sandbox_api).to receive(:get_sandbox).with('sandbox-123').and_return(build_sandbox_dto(state: NightonaApiClient::SandboxState::STOPPED))
    ...
    it 'passes force stop options through to the API' do
      ...
      allow(sandbox_api).to receive(:get_sandbox).with('sandbox-123').and_return(build_sandbox_dto(state: NightonaApiClient::SandboxState::DESTROYED))
  ...
  describe '#resize' do
    it 'raises error when resources is nil' do
      expect { sandbox.resize(nil) }.to raise_error(Nightona::Sdk::Error, /Resources must not be nil/)
    end

    it 'calls resize_sandbox with resource params' do
      allow(sandbox_api).to receive(:resize_sandbox).and_return(build_sandbox_dto(
                                                                  state: NightonaApiClient::SandboxState::STARTED, cpu: 8
                                                                ))

      sandbox.resize(Nightona::Resources.new(cpu: 8, memory: 16, disk: 32), 5)
  ...
  describe '#create_lsp_server' do
    it 'returns an LspServer instance' do
      ...
      expect(lsp).to be_a(Nightona::LspServer)
  ...
  describe '#wait_for_sandbox_start' do
    it 'raises when the sandbox enters an error state' do
      errored = described_class.new(
        sandbox_dto: build_sandbox_dto(state: NightonaApiClient::SandboxState::ERROR, error_reason: 'boom'),
        config: config,
        sandbox_api: sandbox_api
      )
      ...
      end.to raise_error(Nightona::Sdk::Error,
  ...
  describe '#wait_for_sandbox_stop' do
    it 'returns immediately when already destroyed' do
      destroyed = described_class.new(
        sandbox_dto: build_sandbox_dto(state: NightonaApiClient::SandboxState::DESTROYED),
        config: config,
        sandbox_api: sandbox_api
      )
  ...
  describe '#experimental_create_snapshot' do
    it 'creates a snapshot and waits for completion' do
      ...
      allow(sandbox_api).to receive(:get_sandbox).with('sandbox-123').and_return(build_sandbox_dto(state: NightonaApiClient::SandboxState::STARTED))
