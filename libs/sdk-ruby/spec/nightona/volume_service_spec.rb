...
RSpec.describe Nightona::VolumeService do
  ...
  describe '#create' do
    it 'creates a volume and returns a Volume model' do
      ...
      expect(volume).to be_a(Nightona::Volume)
  ...
  describe '#delete' do
    it 'deletes a volume by id' do
      volume = Nightona::Volume.new(build_volume_dto)
  ...
  describe '#get' do
    ...
    it 'creates the volume when not found and create: true' do
      error = NightonaApiClient::ApiError.new(code: 404, message: 'Volume with name missing-vol not found')
    ...
    it 're-raises not found errors when create is false' do
      error = NightonaApiClient::ApiError.new(code: 404, message: 'Volume with name x not found')
      allow(volumes_api).to receive(:get_volume_by_name).and_raise(error)

      expect { service.get('x') }.to raise_error(NightonaApiClient::ApiError)
    end

    it 're-raises unrelated 404 errors even when create is true' do
      error = NightonaApiClient::ApiError.new(code: 404, message: 'some other 404')
      allow(volumes_api).to receive(:get_volume_by_name).and_raise(error)

      expect { service.get('x', create: true) }.to raise_error(NightonaApiClient::ApiError)
    end
  end
  ...
  describe '#list' do
    it 'returns an array of Volume models' do
      ...
      expect(volumes).to all(be_a(Nightona::Volume))
