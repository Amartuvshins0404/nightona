...
RSpec.describe Nightona::SnapshotService do
  ...
  describe '#list' do
    it 'returns PaginatedResource of Snapshots' do
      ...
      expect(result).to be_a(Nightona::PaginatedResource)
      expect(result.items.first).to be_a(Nightona::Snapshot)
      expect(result.total).to eq(1)
    end

    it 'passes pagination params' do
      paginated = instance_double(NightonaApiClient::PaginatedSnapshots, total: 5, page: 2, total_pages: 3, items: [])
    ...
    it 'raises on invalid page' do
      expect { service.list(page: 0) }.to raise_error(Nightona::Sdk::Error, /page must be positive integer/)
    end

    it 'raises on invalid limit' do
      expect { service.list(limit: -1) }.to raise_error(Nightona::Sdk::Error, /limit must be positive integer/)
    end
  end
  ...
  describe '#get' do
    it 'returns Snapshot by name' do
      ...
      expect(snapshot).to be_a(Nightona::Snapshot)
  ...
  describe '#delete' do
    it 'removes snapshot by id' do
      snapshot = Nightona::Snapshot.from_dto(build_snapshot_dto)
  ...
  describe '#activate' do
    it 'activates a snapshot' do
      ...
      expect(result).to be_a(Nightona::Snapshot)
  ...
  describe '#create' do
    it 'creates a snapshot from a string image and uses the default region' do
      created_dto = build_snapshot_dto(state: NightonaApiClient::SnapshotState::ACTIVE)
      allow(snapshots_api).to receive(:create_snapshot).and_return(created_dto)

      params = Nightona::CreateSnapshotParams.new(name: 'my-snap', image: 'ubuntu:22.04', entrypoint: ['/bin/bash'])
    ...
    it 'creates a snapshot from an Image object with resources and image context' do
      image = Nightona::Image.base('python:3.12').entrypoint(['/bin/bash'])
      created_dto = build_snapshot_dto(state: NightonaApiClient::SnapshotState::ACTIVE)
    ...
    it 'streams build logs and emits state transition messages' do
      pending_dto = build_snapshot_dto(id: 'snap-123', name: 'build-snap', state: NightonaApiClient::SnapshotState::PENDING)
      building_dto = build_snapshot_dto(id: 'snap-123', name: 'build-snap', state: 'building')
      active_dto = build_snapshot_dto(id: 'snap-123', name: 'build-snap', state: NightonaApiClient::SnapshotState::ACTIVE)
      ...
      allow(Nightona::Util).to receive(:stream_async).and_return(thread)
      allow(service).to receive(:sleep)

      service.create(Nightona::CreateSnapshotParams.new(name: 'build-snap', image: 'ubuntu:22.04'), on_logs: lambda { |msg|
        on_logs << msg
      })
    ...
    it 'raises on failed snapshot creation' do
      failed_dto = build_snapshot_dto(name: 'fail-snap', state: NightonaApiClient::SnapshotState::BUILD_FAILED,
                                      error_reason: 'dockerfile error')
      allow(snapshots_api).to receive(:create_snapshot).and_return(failed_dto)

      params = Nightona::CreateSnapshotParams.new(name: 'fail-snap', image: 'bad:image')
  ...
  describe '.process_image_context' do
    it 'returns an empty array when no contexts exist' do
      image = Nightona::Image.base('python:3.12')
    ...
    it 'uploads every image context using object storage push access credentials' do
      image = Nightona::Image.base('python:3.12')
      image.context_list << Nightona::Context.new(source_path: '/tmp/a', archive_path: 'a')
      image.context_list << Nightona::Context.new(source_path: '/tmp/b', archive_path: 'b')
      ...
      storage = instance_double(Nightona::ObjectStorage)

      allow(object_storage_api).to receive(:get_push_access).and_return(creds)
      allow(Nightona::ObjectStorage).to receive(:new).and_return(storage)
