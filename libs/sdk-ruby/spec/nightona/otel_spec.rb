...
RSpec.describe Nightona do
  describe Nightona::OtelState do
  ...
  describe '.shutdown_otel' do
    ...
    it 'delegates to the state shutdown method' do
      state = instance_double(Nightona::OtelState)
  ...
  describe '.with_instrumentation' do
    ...
    it 'records a successful duration metric' do
      ...
      state = instance_double(Nightona::OtelState, tracer: tracer)
  ...
  describe Nightona::Instrumentation do
    let(:test_class) do
      Class.new do
        include Nightona::Instrumentation
    ...
    it 'wraps public methods with Nightona.with_instrumentation' do
      otel_state = double('OtelState')
      instance = test_class.new(otel_state)
      allow(Nightona).to receive(:with_instrumentation).and_yield

      expect(instance.ping('value')).to eq('pong value')
      expect(Nightona).to have_received(:with_instrumentation).with(otel_state, 'Dummy', 'ping')
    end
