...
RSpec.describe Nightona::Process do
  ...
  let(:toolbox_api) { instance_double(NightonaToolboxApiClient::ProcessApi, api_client: api_client) }
  ...
  describe '#exec' do
    ...
    it 'executes a command and returns ExecuteResponse' do
      ...
      expect(response).to be_a(Nightona::ExecuteResponse)
  ...
  describe '#code_run' do
    ...
    let(:chart_dto) do
      double('Chart', type: Nightona::ChartType::LINE, title: 'chart', png: 'png', elements: [chart_element],
    ...
    it 'calls ProcessApi#code_run with language, argv, env, and timeout' do
      ...
      expect(result).to be_a(Nightona::ExecuteResponse)
      expect(result.artifacts.charts.first).to be_a(Nightona::LineChart)
  ...
  describe '#execute_session_command' do
    it 'executes a command and returns SessionExecuteResponse' do
      ...
      req = Nightona::SessionExecuteRequest.new(command: 'echo hello', run_async: true, suppress_input_echo: true)
      result = process.execute_session_command(session_id: 'sess-1', req: req)

      expect(result).to be_a(Nightona::SessionExecuteResponse)
  ...
  describe '#get_entrypoint_logs' do
    it 'returns parsed entrypoint logs' do
      ...
      expect(result).to be_a(Nightona::SessionCommandLogsResponse)
  ...
  describe '#get_session_command_logs_async' do
    it 'streams stdout and stderr chunks until close' do
      ...
      expect(WebSocket::Client::Simple).to have_received(:connect).with(
        ...
        headers: hash_including('X-Nightona-Preview-Token' => 'tok')
      )
    end
  end
  ...
  describe '#create_pty_session' do
    it 'creates a PTY session and connects to it' do
      ...
      handle = instance_double(Nightona::PtyHandle)
  ...
  describe '#connect_pty_session' do
    it 'connects via websocket and waits for the PTY connection' do
      ...
      handle = instance_double(Nightona::PtyHandle)
      allow(WebSocket::Client::Simple).to receive(:connect).and_yield(socket).and_return(socket)
      allow(Nightona::PtyHandle).to receive(:new).and_return(handle)
      ...
      expect(Nightona::PtyHandle).to have_received(:new).with(
        socket,
        session_id: 'pty-1',
  ...
  describe '#resize_pty_session' do
    it 'resizes a PTY session' do
      ...
      response = process.resize_pty_session('pty-1', Nightona::PtySize.new(rows: 30, cols: 120))
