...
RSpec.describe Nightona::Nightona do
  ...
  let(:sandbox_api) { instance_double(NightonaApiClient::SandboxApi) }
  let(:config_api) { instance_double(NightonaApiClient::ConfigApi) }
  let(:volumes_api) { instance_double(NightonaApiClient::VolumesApi) }
  let(:object_storage_api) { instance_double(NightonaApiClient::ObjectStorageApi) }
  let(:snapshots_api) { instance_double(NightonaApiClient::SnapshotsApi) }
  let(:sandbox_dto) { build_sandbox_dto }
  let(:sandbox) { instance_double(Nightona::Sandbox, id: 'sandbox-123', state: NightonaApiClient::SandboxState::STARTED) }

  before do
    allow(NightonaApiClient::ApiClient).to receive(:new).and_return(api_client)
    allow(NightonaApiClient::SandboxApi).to receive(:new).and_return(sandbox_api)
    allow(NightonaApiClient::ConfigApi).to receive(:new).and_return(config_api)
    allow(NightonaApiClient::VolumesApi).to receive(:new).and_return(volumes_api)
    allow(NightonaApiClient::ObjectStorageApi).to receive(:new).and_return(object_storage_api)
    allow(NightonaApiClient::SnapshotsApi).to receive(:new).and_return(snapshots_api)
    allow(Nightona::Sandbox).to receive(:new).and_return(sandbox)
  end

  describe '#initialize' do
    it 'creates instance with valid api_key config' do
      nightona = described_class.new(config)

      expect(nightona.config).to eq(config)
      expect(nightona.volume).to be_a(Nightona::VolumeService)
      expect(nightona.snapshot).to be_a(Nightona::SnapshotService)
    end

    it 'configures API client headers and user agent' do
      described_class.new(config)

      expect(api_client.default_headers['X-Nightona-Source']).to eq('sdk-ruby')
      expect(api_client.default_headers['X-Nightona-SDK-Version']).to eq(Nightona::Sdk::VERSION)
      expect(api_client).to have_received(:user_agent=).with("sdk-ruby/#{Nightona::Sdk::VERSION}")
    end

    it 'adds organization header when using a JWT token' do
      jwt_config = Nightona::Config.new(jwt_token: 'jwt', organization_id: 'org-1', api_url: 'https://api.example.com')

      described_class.new(jwt_config)

      expect(api_client.default_headers['X-Nightona-Organization-ID']).to eq('org-1')
    end
    ...
    it 'initializes otel when the otel_enabled config option is set' do
      ...
      allow(Nightona).to receive(:init_otel).and_return(otel_state)

      described_class.new(otel_config)

      expect(Nightona).to have_received(:init_otel).with(Nightona::Sdk::VERSION)
    end
    ...
    it 'initializes otel when NIGHTONA_OTEL_ENABLED is set' do
      env_config = build_config
      allow(env_config).to receive(:read_env).with('NIGHTONA_OTEL_ENABLED').and_return('true')
      allow(env_config).to receive(:read_env).with('NIGHTONA_EXPERIMENTAL_OTEL_ENABLED').and_return(nil)
      allow(Nightona).to receive(:init_otel).and_return(double('OtelState'))

      described_class.new(env_config)

      expect(Nightona).to have_received(:init_otel)
    end

    it 'initializes otel when NIGHTONA_EXPERIMENTAL_OTEL_ENABLED is set' do
      env_config = build_config
      allow(env_config).to receive(:read_env).with('NIGHTONA_OTEL_ENABLED').and_return(nil)
      allow(env_config).to receive(:read_env).with('NIGHTONA_EXPERIMENTAL_OTEL_ENABLED').and_return('true')
      allow(Nightona).to receive(:init_otel).and_return(double('OtelState'))
    ...
    it 'raises error when no api_key or jwt_token provided' do
      bad_config = Nightona::Config.new(api_key: nil, jwt_token: nil, api_url: 'https://api.example.com')

      expect { described_class.new(bad_config) }
        .to raise_error(Nightona::Sdk::Error, /Authentication credentials not found. Set NIGHTONA_API_KEY/)
    end

    it 'raises error when jwt_token without organization_id' do
      bad_config = Nightona::Config.new(jwt_token: 'jwt', organization_id: nil, api_url: 'https://api.example.com')

      expect { described_class.new(bad_config) }
        .to raise_error(Nightona::Sdk::Error, /NIGHTONA_ORGANIZATION_ID is required/)
    end
  end
  ...
  describe '#create' do
    it 'creates a sandbox with default snapshot params and python language when params are nil' do
      ...
      nightona = described_class.new(config)
      result = nightona.create

      expect(result).to eq(sandbox)
      expect(sandbox_api).to have_received(:create_sandbox) do |request|
        expect(request.labels[Nightona::CODE_TOOLBOX_LANGUAGE_LABEL]).to eq('python')
      end
    end

    it 'fills in a default language when params.language is nil' do
      params = Nightona::CreateSandboxFromSnapshotParams.new(snapshot: 'snap-1')
      allow(sandbox_api).to receive(:create_sandbox).and_return(sandbox_dto)
    ...
    it 'raises on invalid language values' do
      params = Nightona::CreateSandboxFromSnapshotParams.new(snapshot: 'snap-1', language: :ruby)
    ...
    it 'raises on negative timeout through the private create helper' do
      ...
      expect { described_class.new(config).send(:_create, params, timeout: -1) }
        .to raise_error(Nightona::Sdk::Error, /Timeout must be a non-negative number/)
    end
    ...
    it 'creates a sandbox from a string image and merges labels' do
      params = Nightona::CreateSandboxFromImageParams.new(
        image: 'ruby:3.4',
        language: :typescript,
        labels: { 'env' => 'test' },
        env_vars: { 'A' => '1' }
      ...
      expect(sandbox_api).to have_received(:create_sandbox) do |request|
        ...
        expect(request.labels).to eq({ 'env' => 'test', Nightona::CODE_TOOLBOX_LANGUAGE_LABEL => 'typescript' })
    ...
    it 'creates a sandbox from an image object and passes resources and network settings' do
      image = Nightona::Image.base('python:3.12').workdir('/workspace')
      params = Nightona::CreateSandboxFromImageParams.new(
        image: image,
        language: :python,
        resources: Nightona::Resources.new(cpu: 2, memory: 4, disk: 8, gpu: 1),
        network_block_all: true,
        network_allow_list: '10.0.0.0/8'
      )
      allow(Nightona::SnapshotService).to receive(:process_image_context).and_return(['hash-1'])
      allow(sandbox_api).to receive(:create_sandbox).and_return(sandbox_dto)

      described_class.new(config).create(params)

      expect(Nightona::SnapshotService).to have_received(:process_image_context).with(object_storage_api, image)
      expect(sandbox_api).to have_received(:create_sandbox) do |request|
    ...
    it 'waits for the sandbox to start when the API returns a non-started state' do
      pending_sandbox = instance_double(Nightona::Sandbox, state: 'pending')
      allow(Nightona::Sandbox).to receive(:new).and_return(pending_sandbox)
    ...
    it 'streams build logs for pending builds when a callback is provided' do
      build_response = build_sandbox_dto(id: 'sb-1', state: NightonaApiClient::SandboxState::PENDING_BUILD)
      started_response = build_sandbox_dto(id: 'sb-1', state: NightonaApiClient::SandboxState::STARTED)
      ...
      allow(Nightona::Util).to receive(:stream_async)
      allow_any_instance_of(described_class).to receive(:sleep)

      described_class.new(config).create(
        Nightona::CreateSandboxFromSnapshotParams.new(snapshot: 'snap-1', language: :python),
        on_snapshot_create_logs: callback
      )

      expect(api_client).to have_received(:update_params_for_auth!).with({}, nil, ['bearer'])
      expect(Nightona::Util).to have_received(:stream_async) do |uri:, headers:, on_chunk:|
  ...
  describe '#get' do
    it 'returns a Sandbox for the given id' do
      ...
      expect(result).to eq(sandbox)
      expect(Nightona::Sandbox).to have_received(:new).with(
  ...
  describe '#list' do
    let(:single_page_response) do
      instance_double(
        NightonaApiClient::ListSandboxesResponse,
        items: [sandbox_dto],
        next_cursor: nil
      )
    end
    ...
    it 'passes label filters and limit hint to the API client' do
      query = Nightona::ListSandboxesQuery.new(labels: { 'env' => 'test' }, limit: 10)
  ...
  describe '#close' do
    it 'shuts down otel and clears the state' do
      otel_state = double('OtelState')
      allow(Nightona).to receive(:init_otel).and_return(otel_state)
      allow(Nightona).to receive(:shutdown_otel)
      nightona = described_class.new(build_config(_experimental: { 'otel_enabled' => true }))

      nightona.close
      nightona.close

      expect(Nightona).to have_received(:shutdown_otel).with(otel_state).once
    end
  end
end
