...
RSpec.describe Nightona::LspServer do
  let(:toolbox_api) { instance_double(NightonaToolboxApiClient::LspApi) }
  let(:lsp) do
    described_class.new(
      language_id: :python,
  ...
  describe 'Language constants' do
    it 'defines supported language constants' do
      expect(Nightona::LspServer::Language::ALL).to eq(%i[javascript python typescript])
    end
  end

  describe 'Position' do
    it 'is a data class with line and character' do
      pos = Nightona::LspServer::Position.new(line: 5, character: 10)
