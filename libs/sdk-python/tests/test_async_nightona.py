...
def _make_async_nightona(config=None):
    from nightona._async.nightona import AsyncNightona
    from nightona_api_client_async import Configuration
    ...
    with (
        ...
        return AsyncNightona(config)


class TestAsyncNightonaInit:
    def test_init_with_config(self):
        nightona = _make_async_nightona(NightonaConfig(api_key="test-key", api_url="https://api.test.io", target="us"))
    ...
    def test_jwt_without_organization_id_raises(self):
        with pytest.raises(NightonaAuthenticationError, match="NIGHTONA_ORGANIZATION_ID is required"):
            _make_async_nightona(NightonaConfig(jwt_token="jwt", api_url="https://api.test.io", target="us"))


class TestAsyncNightonaContextManager:
    @pytest.mark.asyncio
    ...
    async def test_close_shuts_down_tracer_provider(self, env_with_api_key):
        ...
        nightona._tracer_provider = tracer_provider

        await nightona.close()

        tracer_provider.shutdown.assert_called_once()
        nightona._api_client.close.assert_awaited_once()
        nightona._toolbox_api_client.close.assert_awaited_once()


class TestAsyncNightonaSharedSession:
    ...
    async def test_first_request_propagates_session_to_other_rest_client(self, monkeypatch):
        ...
        from nightona.internal import shared_session as ss
        from nightona.internal.shared_session import SharedAiohttpSession
    ...
    def test_retry_backoff_has_jitter_and_grows_with_attempt(self):
        ...
        from nightona.internal.shared_session import (
...
class TestAsyncNightonaCreateValidation:
    @pytest.mark.asyncio
    async def test_negative_timeout_raises(self, env_with_api_key):
        ...
        with pytest.raises(NightonaValidationError, match="Timeout must be a non-negative number"):
            await nightona._create(CreateSandboxFromSnapshotParams(language="python"), timeout=-1)
    ...
    async def test_create_defaults_language_and_sets_label(self, env_with_api_key, sandbox_dto):
        ...
        nightona = _make_async_nightona()
        nightona._sandbox_api.create_sandbox = AsyncMock(return_value=sandbox_dto)
...
class TestAsyncNightonaGetAndList:
    @pytest.mark.asyncio
    ...
    async def test_get_returns_sandbox(self, env_with_api_key, sandbox_dto):
        from nightona._async.sandbox import AsyncSandbox

        nightona = _make_async_nightona()
        nightona._sandbox_api = AsyncMock()
        nightona._sandbox_api.get_sandbox.return_value = sandbox_dto
        sandbox = await nightona.get("test-sandbox-id")
    ...
    def test_list_returns_async_iterator(self, env_with_api_key):
        ...
        assert inspect.isasyncgenfunction(AsyncNightona.list)
    ...
    async def test_list_serializes_labels(self, env_with_api_key, sandbox_dto):
        ...
        nightona = _make_async_nightona()
        nightona._sandbox_api.list_sandboxes = AsyncMock(return_value=response)

        sandboxes = []
        async for sb in nightona.list(ListSandboxesQuery(labels={"project": "test"}, limit=10)):
            sandboxes.append(sb)
    ...
    async def test_list_paginates_via_cursor(self, env_with_api_key, sandbox_dto):
        ...
        nightona = _make_async_nightona()
        nightona._sandbox_api.list_sandboxes = AsyncMock(side_effect=[page1, page2])
    ...
    async def test_list_early_termination_stops_fetching(self, env_with_api_key, sandbox_dto):
        ...
        async for _ in nightona.list():
            break

        assert nightona._sandbox_api.list_sandboxes.call_count == 1


class TestAsyncNightonaValidateLanguageLabel:
    ...
    def test_valid_language_values(self, env_with_api_key, value):
        nightona = _make_async_nightona()
        result = nightona._validate_language_label(value)
...
class TestResolveHappyEyeballsDelay:
    ...
    def test_unset_returns_missing_sentinel(self, raw):
        from nightona._async.nightona import _MISSING_HAPPY_EYEBALLS_DELAY, _resolve_happy_eyeballs_delay
    ...
    def test_none_disables(self, raw):
        from nightona._async.nightona import _resolve_happy_eyeballs_delay
    ...
    def test_invalid_values_raise(self, raw):
        ...
        with pytest.raises(
            ...
            match="NIGHTONA_HAPPY_EYEBALLS_DELAY must be a finite non-negative float or 'none'",
        ):
