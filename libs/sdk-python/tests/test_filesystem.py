...
from nightona.common.filesystem import DownloadProgress, UploadProgress
...
class TestSyncFileSystem:
    def _make_fs(self):
        from nightona._sync.filesystem import FileSystem
    ...
    def test_set_file_permissions(self):
        ...
        fs.set_file_permissions("workspace/script.sh", mode="755", owner="nightona")
        api.set_file_permissions.assert_called_once_with(
            path="workspace/script.sh", mode="755", owner="nightona", group=None
        )
    ...
    def test_download_file_raises_when_response_has_error(self):
        ...
        with pytest.raises(NightonaError, match="missing"):
    ...
    def test_download_file_stream_raises_on_error_part(self):
        ...
        with pytest.raises(NightonaError, match="missing") as exc_info:
    ...
    def test_download_file_stream_cancel_event_aborts(self):
        ...
        cancel.set()
        with pytest.raises(NightonaError, match="cancelled"):
            next(stream)
    ...
    def test_download_file_stream_raises_on_truncated_response(self):
        ...
        with pytest.raises(NightonaError, match=r"[Tt]runcated"):
            list(fs.download_file_stream(remote_path))


class TestAsyncFileSystem:
    def _make_fs(self):
        from nightona._async.filesystem import AsyncFileSystem
    ...
    async def test_download_file_stream_yields_chunks(self):
        ...
        assert streamed_chunks == [b"hello", b" wor", b"ld"]
        assert client.stream_args == ("POST", "https://download")
        from nightona.internal.http_client import aiohttp_request_timeout
    ...
    async def test_upload_file_stream_accepts_async_iterable(self):
        ...
        with patch("nightona._async.filesystem.httpx.AsyncClient", return_value=client):
    ...
    async def test_upload_file_stream_rejects_async_on_progress_with_sync_source(self):
        ...
        with pytest.raises(NightonaError, match="async source"):
            await fs.upload_file_stream(
                b"sync-bytes",
                "/remote/sync.bin",
                on_progress=on_progress,
