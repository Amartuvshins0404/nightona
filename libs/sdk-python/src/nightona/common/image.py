...
from .errors import NightonaNotFoundError, NightonaValidationError
...
class Image(BaseModel):
    ...
    def pip_install_from_requirements(
        ...
        if not req_path.exists():
            raise NightonaNotFoundError(f"Requirements file {requirements_txt} does not exist")
        if not req_path.is_file():
            raise NightonaValidationError(f"Requirements path {requirements_txt} exists but is not a file")
    ...
    def pip_install_from_pyproject(
        ...
        if not pyproject_path.exists():
            raise NightonaNotFoundError(f"pyproject.toml file {pyproject_toml} does not exist")
        if not pyproject_path.is_file():
            raise NightonaValidationError(f"pyproject.toml path {pyproject_toml} exists but is not a file")

        try:
            toml_data: dict[str, object] = toml.load(pyproject_toml)
        except toml.TomlDecodeError as error:
            raise NightonaValidationError(f"Invalid pyproject.toml file {pyproject_toml}: {error}") from error

        project_section = toml_data.get("project")
        if not isinstance(project_section, dict):
            raise NightonaValidationError(
                (
        ...
        if not isinstance(declared_dependencies, list):
            ...
            raise NightonaValidationError(msg)
        ...
        if optional_dependencies:
            ...
            if not isinstance(optionals, dict):
                raise NightonaValidationError("optional-dependencies must be a mapping in pyproject.toml")
    ...
    def add_local_file(self, local_path: str | Path, remote_path: str) -> "Image":
        ...
        lp = Path(local_path)
        if not lp.exists():
            raise NightonaNotFoundError(f"Local file {local_path} does not exist")
        if not lp.is_file():
            raise NightonaValidationError(f"Local path {local_path} exists but is not a file")
    ...
    def add_local_dir(self, local_path: str | Path, remote_path: str) -> "Image":
        ...
        lp = Path(local_path)
        if not lp.exists():
            raise NightonaNotFoundError(f"Local directory {local_path} does not exist")
        if not lp.is_dir():
            raise NightonaValidationError(f"Local path {local_path} exists but is not a directory")
    ...
    def dockerfile_commands(
        ...
        if context_dir:
            ...
            if not os.path.exists(context_dir):
                raise NightonaNotFoundError(f"Context directory {context_dir} does not exist")
            if not os.path.isdir(context_dir):
                raise NightonaValidationError(f"Context path {context_dir} exists but is not a directory")
    ...
    def from_dockerfile(path: str | Path) -> "Image":
        ...
        if not path.exists():
            raise NightonaNotFoundError(f"Dockerfile {path} does not exist")
        if not path.is_file():
            raise NightonaValidationError(f"Dockerfile path {path} exists but is not a file")
    ...
    def __process_python_version(python_version: str | None, allow_micro_granularity: bool = True) -> str:
        ...
        Raises:
            NightonaError: If the Python version is invalid.
        """
        if python_version is None:
            ...
            raise NightonaValidationError(f"Invalid Python version: {python_version!r}")
        else:
        ...
        if series_version not in SUPPORTED_PYTHON_SERIES:
            raise NightonaValidationError(
                (
                    ...
                    f"Nightona supports the following series: {SUPPORTED_PYTHON_SERIES!r}."
                )
            )
