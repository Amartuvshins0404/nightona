...
from .errors import NightonaError, NightonaValidationError, create_nightona_error
...
class CancelEvent(Protocol):
    ...
    ``NightonaError``, closing the underlying HTTP connection."""
...
def create_file_download_error(response: FileDownloadResponse) -> NightonaError:
    """Create the appropriate Nightona exception for a failed file download response."""

    if response.error is None:
        raise NightonaValidationError("response.error must not be None")

    if response.error_details is None:
        return NightonaError(response.error)

    return create_nightona_error(
...
def raise_if_stream_error(
    ...
    if not received_file_data:
        raise NightonaError(f"No file data received for: {remote_path}")
