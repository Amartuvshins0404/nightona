...
async def std_demux_stream_aio(
    ...
    Raises:
        NightonaError: If the WebSocket emits an ERROR frame or closes with a non-1000 code.
    """
    ...
    async def recv() -> bytes | str | None:
        ...
        if msg_type == aiohttp.WSMsgType.ERROR:
            exc = ws.exception()
            raise NightonaError(f"WebSocket error: {exc}") from exc
        if msg_type == aiohttp.WSMsgType.CLOSE:
            ...
            raise NightonaError(f"{detail} (close code {close_code})")
        if msg_type in (aiohttp.WSMsgType.CLOSED, aiohttp.WSMsgType.CLOSING):
            close_code = ws.close_code
            if close_code in (None, 1000):
                return None
            raise NightonaError(f"WebSocket closed unexpectedly (close code {close_code})")
        return None
...
async def std_demux_stream_httpx_ws(
    ...
    EOF, anything else raises ``NightonaError`` so daemon-side failures don't get swallowed.
    """
        ...
        while True:
            try:
                ...
                if e.code in (None, 1000):
                    return None
                raise NightonaError(f"{e.reason or 'WebSocket closed unexpectedly'} (close code {e.code})") from e
            ...
            if isinstance(event, CloseConnection):
                if event.code in (None, 1000):
                    return None
                raise NightonaError(f"{event.reason or 'WebSocket closed unexpectedly'} (close code {event.code})")
