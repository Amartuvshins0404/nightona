...
from ..common.errors import NightonaTimeoutError, NightonaValidationError
...
class _TimeoutMarker(BaseException):
    ...
    to NightonaTimeoutError with a proper error message. We use BaseException
...
def http_timeout(timeout: float | None) -> float | None:
    ...
    - HTTP client times out at 0.2s first → raises NightonaError (wrong!)

    With this fix:
    - timeout=0.2 → HTTP timeout: 1s (matches decorator) → NightonaTimeoutError
    - timeout=5.0 → HTTP timeout: 5s (unchanged) → NightonaTimeoutError
...
class _ThreadingTimeout:
    ...
    This implements a timeout mechanism that raises NightonaTimeoutError in the calling
    ...
    Nested Timeout Handling:
        ...
        - The instance that set `_timed_out=True` converts it to NightonaTimeoutError
    ...
    def __exit__(
        ...
        # If we timed out via our marker exception, convert to NightonaTimeoutError
        if exc_type is _TimeoutMarker:
            if timed_out:
                raise NightonaTimeoutError(
...
class _SignalTimeout:
    ...
    def _timeout_handler(self, _signum: int, _frame: FrameType | None) -> None:
        ...
        raise NightonaTimeoutError(f"Function '{self.func_name}' exceeded timeout of {self.seconds} seconds.")
...
def with_timeout() -> Callable[[F], F]:
    ...
    when a timeout occurs, allowing for proper resource cleanup. The `NightonaTimeoutError` is
    ...
    Platform Support:
        - **Async functions**: All platforms (uses asyncio task cancellation). The exception that can
            ...
            this wrapper converts it to NightonaTimeoutError only after the wrapped method returns.
        - **Sync functions (Unix/Linux, main thread)**: Uses SIGALRM signal. Throws `NightonaTimeoutError`
            directly inside the wrapped method.
        - **Sync functions (Windows or threads)**: Uses threading.Timer with async exception raising.
            Throws `NightonaTimeoutError` directly inside the wrapped method.
    ...
    Raises:
        NightonaTimeoutError: When the function exceeds the specified timeout.
        NightonaValidationError: If timeout is negative.
    ...
    def decorator(func: F) -> F:
        ...
        if inspect.iscoroutinefunction(func):
            ...
            async def async_wrapper(*args: Any, **kwargs: Any) -> Any:
                ...
                if timeout < 0:
                    raise NightonaValidationError("Timeout must be a non-negative number or None.")
                ...
                try:
                    ...
                    # pylint: disable=raise-missing-from
                    raise NightonaTimeoutError(f"Function '{func.__name__}' exceeded timeout of {timeout} seconds.")
