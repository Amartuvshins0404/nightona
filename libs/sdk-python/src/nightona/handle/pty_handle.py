...
from nightona_toolbox_api_client import PtySessionInfo

from ..common.errors import NightonaConnectionError, NightonaError, NightonaTimeoutError
...
class PtyHandle:
    ...
    def wait_for_connection(self, timeout: float = 10.0) -> None:
        ...
        Raises:
            NightonaError: If connection timeout or connection fails
        """
        if self._connection_established:
            return

        if self._ws is None:
            raise NightonaConnectionError("WebSocket connection is not available")

        start_time = time.time()
        while not self._connection_established:
            if time.time() - start_time > timeout:
                raise NightonaTimeoutError("PTY connection timeout")

            if self._error:
                raise NightonaConnectionError(self._error or "Connection failed")
            ...
            try:
                ...
                raise NightonaConnectionError("Connection closed during setup") from e
    ...
    def send_input(self, data: str | bytes) -> None:
        ...
        Raises:
            NightonaError: If PTY is not connected or sending fails
        """
        if not self.is_connected():
            raise NightonaConnectionError("PTY is not connected")
        ...
        try:
            ...
            raise NightonaConnectionError(f"Failed to send input to PTY: {e}") from e
    ...
    def resize(self, pty_size: PtySize) -> PtySessionInfo:
        ...
        Raises:
            NightonaError: If resize handler is not available or resize fails
        """
        if not self._handle_resize:
            raise NightonaError("Resize handler not available")

        try:
            return self._handle_resize(pty_size)
        except Exception as e:
            raise NightonaError(f"Failed to resize PTY: {e}") from e
    ...
    def kill(self) -> None:
        ...
        Raises:
            NightonaError: If kill handler is not available or kill fails
        """
        if not self._handle_kill:
            raise NightonaError("Kill handler not available")

        try:
            self._handle_kill()
        except Exception as e:
            raise NightonaError(f"Failed to kill PTY: {e}") from e
