...
from nightona_toolbox_api_client_async import CreateContextRequest, InterpreterApi, InterpreterContext

from .._utils.errors import intercept_errors
from ..common.code_interpreter import ExecutionError, ExecutionResult, OutputMessage
from ..common.errors import NightonaConnectionError, NightonaTimeoutError
...
class AsyncCodeInterpreter:
    ...
    async def run_code(
        ...
        *,
        ...
        Raises:
            NightonaTimeoutError: If execution times out.
            NightonaError: If execution fails due to communication or other SDK errors.
        ...
        try:
            ...
            while True:
                ...
                if msg.type == aiohttp.WSMsgType.ERROR:
                    exc = ws.exception()
                    raise NightonaConnectionError(f"WebSocket error: {exc}") from exc
        finally:
            if not ws.closed:
                _ = await ws.close()
    ...
    async def create_context(
        ...
        Raises:
            NightonaError: If context creation fails.
    ...
    async def list_contexts(self) -> list[InterpreterContext]:
        ...
        Raises:
            NightonaError: If listing fails.
    ...
    async def delete_context(self, context: InterpreterContext) -> None:
        ...
        Raises:
            NightonaError: If deletion fails or context not found.
    ...
    def _maybe_raise_from_ws_close(self, close_code: int | None, close_reason: str | None) -> None:
        """Translate a websocket close into a Nightona error when it indicates failure.
        ...
        raise NightonaConnectionError(detail)
