...
from nightona_api_client_async import CreateVolume, VolumesApi
from nightona_api_client_async.exceptions import NotFoundException
...
class AsyncVolumeService:
    ...
    async def list(self) -> list[Volume]:
        ...
        Example:
            ...
            async with AsyncNightona() as nightona:
                volumes = await nightona.volume.list()
    ...
    async def get(self, name: str, create: bool = False) -> Volume:
        ...
        Example:
            ```python
            async with AsyncNightona() as nightona:
                volume = await nightona.volume.get("test-volume-name", create=True)
                print(f"{volume.name} ({volume.id})")
            ```
        """
        try:
    ...
    async def create(self, name: str) -> Volume:
        ...
        Example:
            ```python
            async with AsyncNightona() as nightona:
                volume = await nightona.volume.create("test-volume")
    ...
    async def delete(self, volume: Volume) -> None:
        ...
        Example:
            ```python
            async with AsyncNightona() as nightona:
                volume = await nightona.volume.get("test-volume")
