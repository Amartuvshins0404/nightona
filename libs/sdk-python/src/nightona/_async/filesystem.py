...
from ..common.errors import NightonaError
...
class AsyncFileSystem:
    ...
    be performed within a Nightona Sandbox.
    """
    ...
    async def download_file_stream(
        ...
        Args:
            ...
            cancel_event (CancelEvent | None): Optional ``asyncio.Event``-compatible token. When
                set during streaming, the next chunk raises ``NightonaError`` and the underlying
        ...
        Raises:
            NightonaError: If the file does not exist, access is denied, or the download is
        ...
        async def stream_generator() -> AsyncIterator[bytes]:
            ...
            def on_headers_finished() -> None:
                ...
                if not cd_params.get(b"filename"):
                    raise NightonaError("No source path found for this file")
            ...
            async def drain() -> AsyncIterator[bytes]:
                ...
                if cancel_event is not None and cancel_event.is_set():
                    raise NightonaError(f"Download cancelled: {remote_path}")
        ...
        try:
            async with http_session_of(self._api_client.api_client).request(
                ...
                if not (content_type_raw == b"multipart/form-data" and b"boundary" in options):
                    raise NightonaError(f"Unexpected Content-Type: {content_type_raw!r}")
                        ...
                        if event_tag == "begin":
                            ...
                            if mode == "error":
                                ...
                                try:
                                    ...
                                    if source:
                                        ...
                                        raise NightonaError(f"Write failed for unknown file with error {e}") from e
                                    mode = None
                            ...
                            if mode == "error" and error_buffer:
                                ...
                                if source:
                                    ...
                                    raise NightonaError(f"Error happened for unknown file with error {error_text}")
                                error_buffer.clear()
    ...
    async def upload_file_stream(
        ...
        """
        if cancel_event is not None and cancel_event.is_set():
            raise NightonaError(f"Upload cancelled: {remote_path}")
    ...
    async def _upload_async_stream(
        ...
        boundary = "----NightonaUpload" + secrets.token_hex(12)
...
def _open_upload_source(stack: ExitStack, source: object) -> io.IOBase:
    ...
    raise NightonaError(f"Unsupported upload source: {type(source).__name__}")
...
class _CountingUploadReader(io.RawIOBase):
    ...
    def read(self, size: int = -1) -> bytes:
        if self._cancel_event is not None and self._cancel_event.is_set():
            raise NightonaError(f"Upload cancelled: {self._remote_path}")
