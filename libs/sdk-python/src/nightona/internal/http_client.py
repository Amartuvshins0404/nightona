...
def _attach_self_finalizer(client: httpx.Client) -> None:
    ...
    Anchoring the finalizer on the client (rather than on the owning ``Nightona``)
    means the pool survives as long as anything still references it — the
    parent ``Nightona`` or any ``Sandbox`` it returned — and closes
    deterministically only when the last reference drops. This prevents
    ``Nightona().create()``-style usage from prematurely closing the shared
