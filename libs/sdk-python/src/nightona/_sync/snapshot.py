...
from nightona_api_client import CreateBuildInfo, CreateSnapshot
from nightona_api_client import GpuType as SyncGpuType
from nightona_api_client import ObjectStorageApi
from nightona_api_client import SandboxClass as SyncSandboxClass
from nightona_api_client import SnapshotDto, SnapshotsApi, SnapshotState

from .._utils.errors import intercept_errors
from .._utils.otel_decorator import with_instrumentation
from .._utils.stream import process_streaming_response
from .._utils.timeout import with_timeout
from ..common.errors import NightonaError, NightonaValidationError
...
class SnapshotService:
    ...
    def list(self, page: int | None = None, limit: int | None = None) -> PaginatedSnapshots:
        ...
        if page is not None and page < 1:
            raise NightonaValidationError("page must be a positive integer")
    ...
    def delete(self, snapshot: Snapshot) -> None:
        ...
        Example:
            ```python
            nightona = Nightona()
            snapshot = nightona.snapshot.get("test-snapshot")
    ...
    def get(self, name: str) -> Snapshot:
        ...
        Example:
            ```python
            nightona = Nightona()
            snapshot = nightona.snapshot.get("test-snapshot-name")
            print(f"{snapshot.name} ({snapshot.image_name})")
            ```
        """
