...
from nightona_api_client import CreateVolume, VolumesApi
...
class VolumeService:
    ...
    def get(self, name: str, create: bool = False) -> Volume:
        ...
        Example:
            ```python
            nightona = Nightona()
            volume = nightona.volume.get("test-volume-name", create=True)
            print(f"{volume.name} ({volume.id})")
            ```
        """
        try:
    ...
    def create(self, name: str) -> Volume:
        ...
        Example:
            ```python
            nightona = Nightona()
            volume = nightona.volume.create("test-volume")
    ...
    def delete(self, volume: Volume) -> None:
        ...
        Example:
            ```python
            nightona = Nightona()
            volume = nightona.volume.get("test-volume")
