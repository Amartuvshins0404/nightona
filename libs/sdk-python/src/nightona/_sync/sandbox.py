...
from nightona_api_client import BuildInfo, CreateSandboxSnapshot, ForkSandbox, PortPreviewUrl, ResizeSandbox
from nightona_api_client import Sandbox as SandboxDto
from nightona_api_client import (
    SandboxApi,
    SandboxLabels,
    SandboxListItem,
    SandboxState,
...
from nightona_toolbox_api_client import (
    ApiClient,
    ComputerUseApi,
    FileSystemApi,
    GitApi,
...
class Sandbox(SandboxDto):
    ...
    Attributes:
        ...
        snapshot (str | None): Nightona snapshot used to create the Sandbox.
    ...
    def recover(self, timeout: float | None = 60):
        ...
        Raises:
            NightonaError: If timeout is negative. If sandbox fails to recover or times out.
    ...
    def set_autostop_interval(self, interval: int) -> None:
        ...
        Raises:
            NightonaValidationError: If interval is negative
        ...
        """
        if interval < 0:
            raise NightonaValidationError("Auto-stop interval must be a non-negative integer")
    ...
    def set_auto_archive_interval(self, interval: int) -> None:
        ...
        """
        if interval < 0:
            raise NightonaValidationError("Auto-archive interval must be a non-negative integer")
    ...
    def update_network_settings(
        ...
        *,
        ...
        Raises:
            NightonaValidationError: If neither argument is set.
    ...
    def resize(self, resources: Resources, timeout: float | None = 60) -> None:
        ...
        Raises:
            NightonaError: If hot-resize constraints are violated, disk resize is attempted on
    ...
    def wait_for_resize_complete(
        ...
        Raises:
            NightonaError: If timeout is negative. If resize operation times out.
        """
        check_interval = 0.1
        start_time = time.monotonic()
    ...
    def _experimental_fork(self, name: str | None = None, timeout: float | None = 60) -> "Sandbox":
        ...
        Raises:
            NightonaError: If the fork operation fails or times out.

        Example:
            ```python
            sandbox = nightona.get("my-sandbox")
    ...
    def _experimental_create_snapshot(self, name: str, timeout: float | None = 60) -> None:
        ...
        Raises:
            NightonaError: If the snapshot operation fails or times out.
    ...
    def pause(self, timeout: float = 60) -> None:
        ...
        Raises:
            NightonaError: If timeout is negative or the operation fails/times out.
        """
        if timeout < 0:
            raise NightonaError("Timeout must be a non-negative number")
        ...
        while self.state == "pausing":
            ...
            if 0 < remaining <= time.time() - wait_start:
                raise NightonaError(f"Sandbox {self.id} failed to pause within {timeout} seconds")
            time.sleep(check_interval)
