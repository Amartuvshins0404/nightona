 ...
 * - `--nightona` set, no sandbox -> the call FAILS (never run on your host)
 * - `--nightona` off            -> the extension is dormant, Pi's local tool runs
 *
...
export function registerTools(pi: ExtensionAPI, getActive: () => ToolSandbox | null): void {
  ...
  /**
   * Resolve the sandbox for a tool call. With `--nightona` on but no sandbox, we
   * throw rather than run on the host. With `--nightona` off, return null so the
   * local tool runs (the extension is dormant — it never broke normal Pi usage).
   */
  function requireSandbox(): ToolSandbox | null {
    const active = getActive()
    if (active) return active
    if (pi.getFlag('nightona') === true) {
      throw new Error('Nightona sandbox is unavailable — the tool was NOT run on your host. Restart Pi.')
    }
    return null
  }
  ...
  // locally, and Nightona's searchFiles only does basename matching. So we run
  ...
  pi.registerTool({
    ...
    async execute(id, params, signal, onUpdate) {
      const active = requireSandbox()
      if (active) {
        // We can only honor a pre-aborted signal here: Nightona's exec is a
  ...
  pi.registerTool({
    ...
    async execute(id, params, signal, onUpdate) {
      const active = requireSandbox()
      if (active) {
        // See find above: only a pre-aborted signal is honorable; Nightona's
  ...
  pi.registerTool({
    ...
    label: 'Preview URL',
    description:
      'Get the public preview URL for a port served inside the Nightona sandbox. ' +
    ...
    async execute(_id, { port }) {
      const active = getActive()
      if (!active) {
        return { content: [{ type: 'text', text: 'No active Nightona sandbox.' }], details: undefined }
      }
      ...
      const text = sandbox.public
        ...
        : `Preview URL for port ${port}: ${link.url}\n` +
          ...
          `  curl -H "x-nightona-preview-token: ${link.token}" ${link.url}`
      return { content: [{ type: 'text', text }], details: undefined }
    },
  })

  // Route user `!` bash commands to the sandbox. When --nightona is set but no
  // sandbox is available, return an error result so the command is NOT run on the
  // host. With --nightona off, return undefined to let Pi run it locally.
  pi.on('user_bash', () => {
    const active = getActive()
    ...
    // sandbox — passing it to Nightona's exec fails its chdir and surfaces as a
    ...
    if (pi.getFlag('nightona') === true) {
      return {
        result: {
          output: 'Nightona sandbox is unavailable — the command was NOT run on your host. Restart Pi.',
          exitCode: 1,
          cancelled: false,
          truncated: false,
        },
