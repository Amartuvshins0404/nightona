...
/**
 * Resolve the Nightona API key.
 *
 * Order:
 *   1. `NIGHTONA_API_KEY` environment variable (the documented SDK convention).
...
export async function resolveApiKey(ctx: ExtensionContext): Promise<string | undefined> {
  const fromEnv = process.env.NIGHTONA_API_KEY?.trim()
  if (fromEnv) return fromEnv

  if (!ctx.hasUI) return undefined

  const entered = await ctx.ui.input('Nightona API key', 'NIGHTONA_API_KEY — create one at https://app.daytona.io')
