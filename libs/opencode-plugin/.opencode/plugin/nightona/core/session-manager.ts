...
/**
 * Manages Nightona sandbox sessions and persists session-sandbox mappings
...
import { NightonaSandboxGitManager } from '../git/sandbox-git-manager'
import { ProjectDataStorage } from './project-data-storage'
import type { PluginInput } from '@opencode-ai/plugin'
import { toast } from './toast'

export class NightonaSessionManager {
  ...
  async getSandbox(sessionId: string, projectId: string, worktree: string, pluginCtx?: PluginInput): Promise<Sandbox> {
    ...
    if (!this.apiKey) {
      logger.error('NIGHTONA_API_KEY is not set. Cannot create or retrieve sandbox.')
      toast.show({
        title: 'Sandbox error',
        message: 'NIGHTONA_API_KEY is not set. Please set the environment variable to use Nightona sandboxes.',
        variant: 'error',
      })
      throw new Error('NIGHTONA_API_KEY is not set. Please set the environment variable to use Nightona sandboxes.')
    }
    ...
    if (this.isPartiallyInitialized(existing)) {
      ...
      const nightona = new Nightona({ apiKey: this.apiKey })
      const reconnectStart = Date.now()
      logger.info(`Nightona get begin sandboxId=${existing.id}`)
      const sandbox = await nightona.get(existing.id)
      logger.info(`Nightona get done sandboxId=${existing.id} in ${Date.now() - reconnectStart}ms`)
      ...
      if (!branchNumber) {
        try {
          await new NightonaSandboxGitManager(sandbox, this.repoPath).ensureDirectory()
        } catch (err) {
    ...
    logger.info(`Nightona create begin sessionId=${sessionId}`)
    const waitingLog = setTimeout(() => {
      logger.warn(`Nightona create still waiting after ${Date.now() - createStart}ms (sessionId=${sessionId})`)
    }, 15_000)
    const sandbox = await nightona.create().finally(() => clearTimeout(waitingLog))
    logger.info(`Nightona create done sessionId=${sessionId} sandboxId=${sandbox.id} in ${Date.now() - createStart}ms`)
    this.sessionSandboxes.set(sessionId, sandbox)
  ...
  async deleteSandbox(sessionId: string, projectId: string): Promise<void> {
    ...
    if (!sandbox || this.isPartiallyInitialized(sandbox)) {
      ...
      if (sessionInfo?.sandboxId) {
        const nightona = new Nightona({ apiKey: this.apiKey })
        try {
          sandbox = await nightona.get(sessionInfo.sandboxId)
          this.sessionSandboxes.set(sessionId, sandbox)
        } catch (err) {
