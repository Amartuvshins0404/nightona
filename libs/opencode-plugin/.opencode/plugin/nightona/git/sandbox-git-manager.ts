...
export class NightonaSandboxGitManager {
  constructor(
  ...
  async ensureRepo(): Promise<void> {
    ...
    if (isGit.trim() !== 'true') {
      ...
      await this.runGitCommand('git config user.name "Nightona Sandbox"')
  ...
  async autoCommit(): Promise<boolean> {
    ...
    await this.runGitCommand('git commit -am "Auto-commit from Nightona plugin"')
