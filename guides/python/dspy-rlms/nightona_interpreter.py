...
# =============================================================================
# NightonaInterpreter - Implements CodeInterpreter protocol using Nightona
# =============================================================================

# Python setup code injected into the Nightona context.
...
class NightonaInterpreter:
    """Remote interpreter using Nightona sandbox for Python execution.

    Implements the CodeInterpreter protocol for secure code execution in a
    Nightona cloud sandbox. The sandbox provides isolated execution with
    persistent state across calls.

    Prerequisites:
        - Nightona SDK: pip install nightona
        - NIGHTONA_API_KEY environment variable set

    Example:
        ```python
        # Basic execution
        with NightonaInterpreter() as interp:
    ...
    def __init__(
        ...
        Args:
            ...
            sandbox_params: Optional parameters to pass to Nightona sandbox creation.
        ...
        self._broker_url = None  # Preview URL for broker (e.g., https://xxx.preview.nightona.work)
    ...
    def start(self) -> None:
        """Initialize the Nightona sandbox and interpreter context."""
        if self._initialized:
            return

        try:
            from nightona import Nightona, SessionExecuteRequest  # pylint: disable=import-outside-toplevel

            self._session_execute_request = SessionExecuteRequest
        except ImportError as exc:
            raise CodeInterpreterError(
                "Nightona not installed. Install with: pip install nightona\n"
                "Also ensure NIGHTONA_API_KEY environment variable is set."
            ) from exc

        logger.info("Creating Nightona sandbox...")
        self._nightona = Nightona()
        self._sandbox = self._nightona.create(**self.sandbox_params)
...
"""
    ...
    def _wait_for_broker_health(self, timeout: float = 30.0) -> None:
        ...
        headers = {"x-nightona-preview-token": self._broker_token}
    ...
    def execute(
        ...
        """Execute Python code in the Nightona sandbox.
    ...
    def shutdown(self) -> None:
        """Release resources and delete the Nightona sandbox."""
