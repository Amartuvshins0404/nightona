# Serving vLLM on GPU Sandboxes (vLLM + Nightona)

## Overview

This guide demonstrates how to serve an open-weights model with [vLLM](https://github.com/vllm-project/vllm) on a Nightona GPU sandbox and query it from anywhere through a token-authenticated preview URL. The server speaks the OpenAI-compatible API, so any OpenAI client works against it unchanged.
...
- **Token-authenticated preview URL:** The endpoint is reachable from anywhere; requests authenticate with the `x-nightona-preview-token` header
...
- **Nightona:** GPU sandboxes are currently experimental, make sure your organization has access to GPUs
...
- `NIGHTONA_API_KEY`: Required for Nightona sandbox access. Get it from [Nightona Dashboard](https://app.daytona.io/dashboard/keys)
...
sb = Nightona(NightonaConfig(target="us-east-1")).get("SANDBOX_ID")
...
```bash
python -c "from nightona import Nightona; Nightona().get('SANDBOX_ID').delete()"
```
...
- `TARGET`: Nightona region; `us-east-1` is currently the region for GPU sandboxes
...
5. **Query:** Clients hit the OpenAI-compatible API through the preview URL, authenticating with the `x-nightona-preview-token` header
