...
func TestServeWarningPage_EscapesXSSPayloadInBody(t *testing.T) {
	...
	c.Request.Host = "nightonaproxy01.net"
...
func TestServeWarningPage_RendersBenignURLReadably(t *testing.T) {
	...
	c.Request.Host = "3000-abc.nightonaproxy01.net"

	serveWarningPage(c, true)

	body := w.Body.String()
	if !strings.Contains(body, "https://3000-abc.nightonaproxy01.net/dashboard/index.html") {
...
func TestHandleAcceptProxyWarning_AllowsSameHostAbsoluteRedirect(t *testing.T) {
	...
	c.Request = httptest.NewRequest(
		"POST",
		ACCEPT_PREVIEW_PAGE_WARNING_PATH+"?redirect=https%3A%2F%2F3000-abc.nightonaproxy01.net%2Fdashboard%2Findex.html",
		nil,
	)
	c.Request.Host = "3000-abc.nightonaproxy01.net"
	...
	if loc := w.Header().Get("Location"); loc != "https://3000-abc.nightonaproxy01.net/dashboard/index.html" {
...
func TestHandleAcceptProxyWarning_AllowsSameHostWithForwardedHostHeader(t *testing.T) {
	// Custom Preview Proxy: the consent POST reaches Nightona on a nightona proxy
	...
	c.Request = httptest.NewRequest(
		"POST",
		ACCEPT_PREVIEW_PAGE_WARNING_PATH+"?redirect=https%3A%2F%2F3000-abc.nightonaproxy01.net%2Fapp",
		nil,
	)
	c.Request.Host = "3000-abc.nightonaproxy01.net"
	...
	if loc := w.Header().Get("Location"); loc != "https://3000-abc.nightonaproxy01.net/app" {
...
func TestHandleAcceptProxyWarning_RejectsOpenRedirectTargets(t *testing.T) {
	cases := []struct {
		...
		{"userinfo-confusion", "https://3000-abc.nightonaproxy01.net@evil.com"},
