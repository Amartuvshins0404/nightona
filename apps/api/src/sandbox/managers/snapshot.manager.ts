...
import { SnapshotInfoResponse } from '@nightona-co/runner-api-client'
...
export class SnapshotManager implements TrackableJobExecutions, OnApplicationShutdown {
  ...
  async cleanupOldBuildInfoSnapshotRunners() {
    ...
    try {
      ...
      const staleEntries = await this.snapshotRunnerRepository
        ...
        .andWhere("sr.snapshotRef LIKE 'nightona-%'")
        .limit(500)
        .getMany()
  ...
  private async processSnapshotDigest(
    ...
    if (!snapshot.ref) {
      ...
      updateData.ref = `${sanitizedUrl}/${internalRegistry.project || 'nightona'}/nightona-${hash}:nightona`
    }
