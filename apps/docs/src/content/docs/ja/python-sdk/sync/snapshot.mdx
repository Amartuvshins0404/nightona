...
事前に構成されたサンドボックスである Nightona のスナップショットを表します。
...
Nightonaのスナップショットを管理するサービス。スナップショットの一覧、取得、作成、削除に使用できます。
...
```python
nightona = Nightona()
snapshots = nightona.snapshot.list()
...
- `image` _Union[str, Image]_ - スナップショットのイメージ。文字列が指定された場合は、
  ...
  Nightona で新しいイメージを作成するために使用されます。
