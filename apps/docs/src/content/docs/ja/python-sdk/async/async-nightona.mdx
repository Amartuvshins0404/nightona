...
Nightona API と対話するためのメインクラス。

このクラスは、Nightona サンドボックスの作成・管理・操作を非同期メソッドで提供します。
...
#### AsyncNightona.__init__

```python
def __init__(config: Optional[NightonaConfig] = None)
```

任意の設定で Nightona インスタンスを初期化します。

config が指定されていない場合、次の環境変数から読み取ります:
- `NIGHTONA_API_KEY`: 認証に必要な API キー（必須）
- `NIGHTONA_API_URL`: API の URL（必須）
- `NIGHTONA_TARGET`: ターゲット環境（任意、デフォルトは 'us'）

**引数**:

- `config` _Optional[NightonaConfig]_ - api_key、api_url、target を含むオブジェクト。
  

**送出**:

- `NightonaError` - API キーが config または環境変数で提供されていない場合
...
このメソッドは AsyncNightona インスタンスの使用を終えたときに呼び出し、
...
- `params` _Optional[CreateSandboxFromSnapshotParams]_ - サンドボックス作成用のパラメータ。指定しない場合は、
  デフォルトの Nightona スナップショットと Python 言語が使用されます。
...
- `NightonaError` - timeout、auto_stop_interval、または auto_archive_interval が負の場合、
...
指定したレジストリで利用可能なイメージ、または宣言的な Nightona イメージからサンドボックスを作成します。
リソース、言語、イメージ、環境変数、ボリュームなど、さまざまなパラメータを指定できます。Nightona は指定されたイメージからスナップショットを作成し、それを用いてサンドボックスを作成します。
...
- `NightonaError` - サンドボックスの削除に失敗した場合、またはタイムアウトした場合
...
sandbox = await nightona.create()
# ... use sandbox ...
await nightona.delete(sandbox)  # 完了時にクリーンアップ
```
...
- `NightonaError` - sandbox_id が指定されていない場合。
  

**例**:

```python
sandbox = await nightona.get("my-sandbox-id")
...
```python
sandboxes = await nightona.list(labels={"my-label": "my-value"})
...
- `NightonaError` - タイムアウトが負の値の場合、またはサンドボックスの起動に失敗した場合・タイムアウトした場合
...
- `NightonaError` - タイムアウトが負の値の場合、またはサンドボックスの停止に失敗した場合・タイムアウトした場合
...
Nightona クライアントを初期化するための設定オプション。

**属性**:

- `api_key` _Optional[str]_ - Nightona API への認証に使用する API キー。未設定の場合は
  環境変数 `NIGHTONA_API_KEY` で指定するか、代わりに JWT トークンを指定する必要があります。
- `jwt_token` _Optional[str]_ - Nightona API への認証に使用する JWT トークン。未設定の場合は
  環境変数 `NIGHTONA_JWT_TOKEN` で指定するか、代わりに API キーを指定する必要があります。
- `organization_id` _Optional[str]_ - JWT ベース認証で使用する組織 ID。JWT トークンが指定されている場合は必須で、
  ここで指定するか、環境変数 `NIGHTONA_ORGANIZATION_ID` に設定する必要があります。
- `api_url` _Optional[str]_ - Nightona API の URL。ここまたは環境変数 `NIGHTONA_API_URL` で未設定の場合は、
...
- `target` _Optional[str]_ - サンドボックスのターゲットとなるランナーのロケーション。ここまたは
  環境変数 `NIGHTONA_TARGET` で未設定の場合は、既定で `'us'` になります。
