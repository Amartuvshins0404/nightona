...
Nightonaのボリューム（S3互換オブジェクトストレージをバックエンドとするFUSEベースの共有ストレージマウント）を管理するサービス。
...
Nightona（サンドボックスを作成・管理するプラットフォーム）のボリューム（S3互換オブジェクトストレージをバックエンドとするFUSEベースの共有ストレージマウント）を表します。サンドボックス（Nightonaが管理する隔離された一時的なコンピュート環境）間で共有されるストレージです。
