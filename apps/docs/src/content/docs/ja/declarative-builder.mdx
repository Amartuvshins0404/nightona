...
Nightona（サンドボックスの作成・管理プラットフォーム）の宣言的ビルダーは、サンドボックス（Nightonaが管理する隔離された一時的な実行環境）の依存関係をコードファーストで定義する強力な手法を提供します。コンテナレジストリからイメージを取り込む代わりに、SDKを用いてプログラム的に定義できます。

## 概要

宣言的ビルダーシステムは、主に次の2つのワークフローをサポートします。

1. **動的イメージ**: サンドボックス（Nightonaが管理する隔離された一時的な実行環境）作成時に、依存関係の異なるイメージをオンデマンドでビルド
...
これにより、ビルド処理のために自前のコンピュートリソースを使う必要がなくなり、Nightonaのインフラストラクチャにオフロードできます。\
...
<Tabs syncKey="language">
  <TabItem label="Python" icon="seti:python">
    ...
    dynamic_image = (
        ...
        .run_commands("apt-get update && apt-get install -y git curl", "mkdir -p /home/nightona/project")
        .workdir("/home/nightona/project")
        .env({"ENV_VAR": "My Environment Variable"})
        .add_local_file("file_example.txt", "/home/nightona/project/file_example.txt")
    )
  ...
  <TabItem label="TypeScript" icon="seti:typescript">
    ...
    const dynamicImage = Image.debianSlim('3.13')
        ...
        .addLocalFile('file_example.txt', '/home/nightona/project/file_example.txt')
...
つまり、毎回同じスクリプトを使えば、Nightonaがイメージのキャッシュを適切に処理します。
:::

## 事前ビルド済みスナップショットの作成

特定の依存関係を含む新しいNightonaのスナップショットを準備し、必要なときに複数のサンドボックスで即座に使いたい場合は、事前ビルド済みのスナップショットを作成できます。

このスナップショットはNightonaダッシュボード上に表示され続け、永続的にキャッシュされるため、再ビルドは不要です。
