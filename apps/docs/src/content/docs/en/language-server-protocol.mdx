...
Nightona provides Language Server Protocol (LSP) support through sandbox instances. This enables advanced language features like code completion, diagnostics, and more.

## Create LSP servers

Nightona provides methods to create LSP servers. The `path_to_project` argument is relative to the current sandbox working directory when no leading `/` is used. The working directory is specified by WORKDIR when it is present in the Dockerfile, and otherwise falls back to the user's home directory.

<Tabs syncKey="language">
<TabItem label="Python" icon="seti:python">

```python
from nightona import Nightona, LspLanguageId
...
```typescript
import { Nightona, LspLanguageId } from '@nightona-co/sdk'
...
The supported languages for creating LSP servers with Nightona are defined by the `LspLanguageId` enum:
...
Nightona provides methods to start LSP servers.
...
Nightona provides methods to stop LSP servers.
...
Nightona provides methods to get code completions for a specific position in a file.
...
Nightona provides methods to notify the LSP server when files are opened or closed. This enables features like diagnostics and completion tracking for the specified files.
...
Nightona provides methods to retrieve symbols (functions, classes, variables, etc.) from a document.
...
Nightona provides methods to search for symbols across all files in the sandbox.
