...
[Nightona Limits ↗](https://app.daytona.io/dashboard/limits) provide an overview of your organization's [current usage](#current-usage), [resources](#resources), [sandbox limits](#sandbox-limits), and [rate limits](#rate-limits). Nightona uses a [tier-based](#tiers) system where organizations are placed into tiers based on verification status, with each tier providing access to a specific compute pool and rate limits. For information on spending and wallet management, see [billing](/docs/en/billing).
...
Nightona supports managing your sandbox resources by [changing the sandbox state](/docs/sandboxes#sandbox-lifecycle). The table below summarizes how each state affects sandbox resource usage:
...
Nightona includes rate limit information in API response headers. Header names include a suffix based on which rate limit is triggered (e.g., `-anonymous`, `-authenticated`, `-sandbox-create`, `-sandbox-lifecycle`):
...
Nightona [Python](/docs/python-sdk), [TypeScript](/docs/en/typescript-sdk), [Ruby](/docs/ruby-sdk) and [Go](/docs/go-sdk) SDKs raise or throw a `NightonaRateLimitError` exception (Python) or error (TypeScript, Ruby and Go) when you exceed a rate limit.
...
<Tabs>
  ...
  <TabItem label="Python" icon="seti:python">
    ```python
    try:
      nightona.create(snapshot="my-snapshot")
    except NightonaRateLimitError as e:
  ...
  <TabItem label="Go" icon="seti:go">
    ```go
    sandbox, err := nightona.Create(ctx, nil)
    if err != nil {
      var rateLimitErr *errors.NightonaRateLimitError
...
Limits are applied to your organization's default region. To unlock higher limits, complete the following verification steps in the [Nightona Dashboard ↗](https://app.daytona.io/dashboard/limits):
...
async function createSandboxWithRetry() {
  ...
  while (retries < maxRetries) {
    try {
      return await nightona.create({ snapshot: 'my-snapshot' })
    } catch (error) {
      if (error instanceof NightonaRateLimitError && retries < maxRetries - 1) {
