...
- `snapshot?` _string_ - Nightona snapshot used to create the Sandbox
...
- `NightonaError` - If Sandbox fails to recover or times out
...
- `NightonaError` - If interval is not a non-negative integer
...
- `NightonaError` - If Sandbox fails to start or times out
...
- `NightonaError` - If the sandbox ends up in an error state or fails to start within the timeout period.
...
- `NightonaError` - If the sandbox fails to stop within the timeout period.
## ListSandboxesQuery
