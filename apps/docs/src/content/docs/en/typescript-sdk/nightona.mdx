...
- `snapshot` _SnapshotService_ - Service for managing Nightona Snapshots
- `volume` _VolumeService_ - Service for managing Nightona Volumes
...
- `config?` _NightonaConfig_ - Configuration options
...
- `apiKey?` _string_ - API key for authentication with the Nightona API
- `apiUrl?` _string_ - URL of the Nightona API. Defaults to 'https://app.daytona.io/api'
    if not set here and not set in environment variable NIGHTONA_API_URL.
- `jwtToken?` _string_ - JWT token for authentication with the Nightona API. If not set, it must be provided
