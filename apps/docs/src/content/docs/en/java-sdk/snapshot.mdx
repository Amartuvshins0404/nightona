...
- `io.nightona.sdk.exception.NightonaException` - if the API request fails
...
- `NightonaException` - if the API request fails or the build fails
...
- `name` _String_ - snapshot name
- `image` _Image_ - declarative image definition
- `resources` _io.nightona.sdk.model.Resources_ - CPU/GPU/memory/disk resources; `null` for defaults
- `onLogs` _Consumer\<String\>_ - callback for build log lines; `null` to skip streaming
...
- `io.nightona.sdk.exception.NightonaException` - if no snapshot is found or request fails
