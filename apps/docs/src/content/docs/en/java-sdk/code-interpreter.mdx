...
- `NightonaException` - if code is empty, connection fails, or execution fails
...
- `NightonaException` - if context creation fails
...
- `NightonaException` - if listing contexts fails
