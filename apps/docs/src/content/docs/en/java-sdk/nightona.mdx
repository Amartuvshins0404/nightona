...
Reads `NIGHTONA_API_KEY`, `NIGHTONA_API_URL`, and `NIGHTONA_TARGET`.
For backward compatibility, the legacy `DAYTONA_API_KEY`, `DAYTONA_API_URL`,
and `DAYTONA_TARGET` variables are used as fallbacks when the `NIGHTONA_`
variables are unset or empty.

**Throws**:

- `NightonaException` - if required authentication is missing
...
- `config` _NightonaConfig_ - SDK configuration containing API key and endpoint settings

**Throws**:

- `NightonaException` - if configuration is invalid or missing credentials
...
- `NightonaException` - if creation or startup fails
...
- `NightonaException` - if creation fails or the Sandbox does not start in time
...
- `NightonaException` - if the Sandbox is not found or request fails
...
`Nightona.list`" on `Sandbox` (env, networkBlockAll, networkAllowList, volumes,
