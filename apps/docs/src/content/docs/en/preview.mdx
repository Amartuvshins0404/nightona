...
Nightona provides preview URLs for accessing services running in your sandboxes. Any process listening for HTTP traffic on ports `1` - `65535` can be previewed through a generated URL.

Nightona supports two types of preview URLs, each with a different authentication mechanism:
...
:::note
Standard and signed preview tokens are not interchangeable. The token from `get_preview_link()` is used as a preview access token (sent via the `x-nightona-preview-token` header). The token from `create_signed_preview_url()` is embedded in the URL itself: it cannot be used as a standard preview token, and vice versa.
:::
...
URL structure: `https://{port}-{sandboxId}.{nightonaProxyDomain}`
...
response = requests.get(
    preview_info.url,
    headers={"x-nightona-preview-token": preview_info.token}
)
```
...
const response = await fetch(previewInfo.url, {
  headers: { 'x-nightona-preview-token': previewInfo.token }
});
```
...
Authenticate by sending the token in the `x-nightona-preview-token` header:

```bash
curl -H "x-nightona-preview-token: vg5c0ylmcimr8b_v1ne0u6mdnvit6gc0" \
...
URL structure: `https://{port}-{token}.{nightonaProxyDomain}`
...
```bash
nightona preview-url <sandbox-name> --port 3000 --expires 3600
```
...
```bash
curl https://3000-<value>.proxy.nightona.work
```
...
When opening a preview link in a browser for the first time, Nightona displays a warning page. This warning informs users about potential risks of visiting the preview URL and only appears when loading the link in a browser.

To skip the warning page:

- Send the `X-Nightona-Skip-Preview-Warning: true` header
