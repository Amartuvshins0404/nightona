...
#### AsyncNightona.\_\_init\_\_

```python
def __init__(config: NightonaConfig | None = None)
```
...
- `config` _NightonaConfig | None_ - Object containing api_key, api_url, and target.
...
- `api_key` _str | None_ - API key for authentication with the Nightona API. If not set, it must be provided
  via the environment variable `NIGHTONA_API_KEY`, or a JWT token must be provided instead.
- `jwt_token` _str | None_ - JWT token for authentication with the Nightona API. If not set, it must be provided
...
- `api_url` _str | None_ - URL of the Nightona API. Defaults to `'https://app.daytona.io/api'` if not set
