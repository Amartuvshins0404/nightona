...
VNC (Virtual Network Computing) access provides a graphical desktop environment for your Nightona Sandbox directly in the browser. This allows you to interact with GUI applications, desktop tools, and visual interfaces running inside your sandbox.
...
Access the VNC desktop environment directly from the [Nightona Dashboard ↗](https://app.daytona.io/dashboard/sandboxes).
...
Nightona provides methods to [start](#start-vnc), [stop](#stop-vnc), and [monitor](#get-vnc-status) VNC sessions and processes programmatically using the [Computer Use](/docs/en/computer-use) references as part of automated workflows.
