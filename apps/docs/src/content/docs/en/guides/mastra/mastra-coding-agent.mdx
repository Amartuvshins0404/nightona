---
title: Use Mastra Coding Agent with Nightona
description: Learn how to integrate Nightona sandboxes with the Mastra coding agent for secure, isolated AI-powered development.
---

This guide demonstrates how to configure the [Mastra coding agent](https://github.com/mastra-ai/template-coding-agent) to use Nightona sandboxes, enabling you to leverage AI capabilities for any coding-related task in a secure, isolated environment.
...
Clone the Mastra coding agent template repository, which includes the agent implementation and Nightona integration:
...
- **Sandbox Provider**: Nightona configuration for isolated execution
...
Next, configure Nightona as your sandbox provider by adding your API key (available from the [Nightona Dashboard](https://app.daytona.io/dashboard/keys)):
...
Let's walk through a practical example to see the Mastra coding agent and Nightona sandbox integration in action. We'll ask the agent to complete the following task:

> _"Create a Python file with a function that reverses a string, write a test file for it, and run the tests."_

#### Agent Workflow

The agent executes this task through a series of coordinated steps, utilizing different tools to interact with the Nightona sandbox:

##### 1. Create Sandbox

The agent first provisions a new Nightona sandbox for Python development:
...
```json
{
  "success": true,
  "filesWritten": [
    "/home/nightona/reverse_string.py",
    "/home/nightona/test_reverse_string.py"
  ]
}
```
...
This example demonstrates how the agent autonomously handles the entire development workflow, from environment setup to test execution, all within the secure Nightona sandbox environment.

:::tip[Sandbox Management]
After the agent completes its task, you'll see the active sandbox in your [Nightona Dashboard](https://app.daytona.io/dashboard). It's recommended to clean up resources when finished, unless your use case requires the sandbox to remain active (e.g. for preview URLs or ongoing development).
:::

**Key Advantages**

- **Secure Isolation**: All agent operations run in isolated Nightona sandboxes, protecting your local environment
...
- **Scalable Execution**: Leverage Nightona's cloud infrastructure for resource-intensive tasks
