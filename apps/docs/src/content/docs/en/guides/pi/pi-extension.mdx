---
title: Run Pi with the Nightona Extension
description: Use the Nightona Pi extension to run every Pi tool call inside a remote Nightona sandbox, with each session synced to its own GitHub branch.
---

import piExtensionGif from '../../../../../assets/docs/images/pi-extension.gif'

This guide demonstrates how to run the [Nightona Pi extension](https://www.npmjs.com/package/@nightona-co/pi) which integrates Nightona sandboxes and [Pi](https://pi.dev/). When the extension is active, the agent runs on your machine while every tool call — `bash`, file I/O, and search — runs inside a secure Nightona sandbox. Each session gets its own sandbox, and your work is synced to a per-session GitHub branch.

### 1. Workflow Overview

When you launch Pi with `--nightona`, a sandbox is created for the session and the repo you launched Pi in is cloned into it. From then on, every tool call — running code, installing dependencies, starting servers — happens in the sandbox rather than on your host.

<img
  src={piExtensionGif.src}
  alt="Pi running with the Nightona extension — sandbox status and a synced GitHub branch"
  width="600"
...
Add the Nightona extension to Pi:
...
| **Git & GitHub**  | GitHub authorization with `gh`, and `/merge` via the GitHub API | Git operations (clone and push) via the [Nightona git API](/docs/en/git-operations) |
...
Each built-in tool is re-registered so that, when a sandbox is active, it runs against it through the [Nightona SDK](https://www.npmjs.com/package/@nightona-co/sdk). A small `sandboxTool` helper wraps a local tool with a per-call rebuild against the active sandbox:
...
function sandboxTool(local, makeRemote) {
  return {
    ...local,
    execute: (...args) => {
      const active = requireSandbox(); // throws when --nightona is set but no sandbox; null when off
...
pi.on("session_start", async (event, ctx) => {
  ...
  active = prev
    ? await nightona.get(prev.sandboxId) // resume → reattach
    : await nightona.create({
...
When you're in a github.com repo and authenticated with `gh`, each session gets its own branch. The host only uses `gh` to mint a token and call the GitHub REST API (to create the branch and merge it); all git transfer — clone, commit, push — runs **inside the sandbox** via the Nightona git API.
...
Pi has no "session deleted" event, so the extension reconciles: on `session_start` and `session_shutdown` it reaps any sandbox whose session no longer exists by comparing Nightona's labelled sandboxes against Pi's live sessions.

```typescript
async function reapOrphans(nightona) {
...
- Secure, isolated execution: every Pi tool call runs in a Nightona sandbox, and never on your host when `--nightona` is set
