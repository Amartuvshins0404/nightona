...
Each built-in tool is re-registered so that, when a sandbox is active, it runs against it through the [Nightona SDK](https://www.npmjs.com/package/@nightona/sdk). A small `sandboxTool` helper wraps a local tool with a per-call rebuild against the active sandbox:
