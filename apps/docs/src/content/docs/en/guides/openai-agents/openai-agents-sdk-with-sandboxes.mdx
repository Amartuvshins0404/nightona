---
title: Using the OpenAI Agents SDK with Nightona Sandboxes
description: "Core patterns for using the OpenAI Agents SDK with Nightona sandboxes: shell access, handoffs, memory, and human-in-the-loop workflows."
---

import { TabItem, Tabs } from '@astrojs/starlight/components'

This guide walks through the core patterns for running AI agents in isolated cloud sandboxes using the [OpenAI Agents SDK](https://developers.openai.com/api/docs/guides/agents) and Nightona. We start from a simple example and progressively layer on multi-agent handoffs, memory, structured outputs, and human-in-the-loop workflows.

See also the [Text-to-SQL Agent with the OpenAI Agents SDK and Nightona](/docs/en/guides/openai-agents/text-to-sql-agent-openai-agents-sdk) guide for a complete project built on these patterns.

---

## Prerequisites

Install the Agents SDK with the Nightona extra:

```shell
pip install openai-agents[nightona]
```
...
export NIGHTONA_API_KEY=...        # from https://app.daytona.io/dashboard/keys
```
...
<Tabs>
  <TabItem label="Python" icon="seti:python">
    ...
    from agents.extensions.sandbox import NightonaSandboxClient, NightonaSandboxClientOptions

    NIGHTONA_ROOT = "/home/nightona/workspace"

    # Declare workspace contents declaratively
    # Use Nightona's home directory as root instead of the default /workspace.
    manifest = Manifest(root=NIGHTONA_ROOT, entries={
    ...
    client = NightonaSandboxClient()
    run_config = RunConfig(
        sandbox=SandboxRunConfig(client=client, options=NightonaSandboxClientOptions())
    )
...
- **`NightonaSandboxClient`** provisions a remote cloud sandbox.  
...
<Tabs>
  <TabItem label="Python" icon="seti:python">
    ```py
    client = NightonaSandboxClient()
    session = await client.create(manifest=manifest, options=NightonaSandboxClientOptions())
    await session.start()
...
By default, when a session shuts down the sandbox is deleted. Setting `pause_on_exit=True` changes this: on shutdown, the SDK calls Nightona's pause API (`sandbox.stop()`) instead of `sandbox.delete()`. The sandbox stays on Nightona's infrastructure in a paused state, preserving the filesystem (including any installed packages).

To reconnect on the next run, you need two things:

1. **Nightona keeps the sandbox alive**, paused on their side, identifiable by its sandbox ID.  
2. **Your code remembers the sandbox ID**. The SDK captures this in `NightonaSandboxSessionState`, a Pydantic model you serialize to disk.

When you call `client.resume(saved_state)`, the SDK uses the `sandbox_id` from that state to call `nightona.get(sandbox_id)`. If the sandbox is still there, it calls `sandbox.start()` to wake it. The workspace is already populated, so it skips full manifest apply but still reapplies ephemeral state (like environment variables) and restores snapshots if needed. If the sandbox has expired or been deleted, `resume()` falls through and creates a fresh one from the same config.
...
<Tabs>
  <TabItem label="Python" icon="seti:python">
    ...
    from agents.extensions.sandbox import (
        NightonaSandboxClient,
        NightonaSandboxClientOptions,
        NightonaSandboxSessionState,
    )

    STATE_FILE = Path(".session_state.json")

    client = NightonaSandboxClient()
    options = NightonaSandboxClientOptions(pause_on_exit=True)

    # Try to resume a previously paused sandbox
    session = None
    if STATE_FILE.exists():
        saved = NightonaSandboxSessionState.model_validate_json(STATE_FILE.read_text())
...
The Agents SDK also has its own **workspace persistence** mechanism (`persist_workspace`/`hydrate_workspace`) that tars up workspace files and saves them externally (local disk, S3). This is useful when the sandbox itself is gone and you need to restore contents into a new one. It's distinct from **Nightona snapshots** (`sandbox_snapshot_name`), which are pre-built sandbox templates you create sandboxes *from*.
...
<Tabs>
  <TabItem label="Python" icon="seti:python">
    ...
    manifest = Manifest(root="/home/nightona/workspace", entries={
    ...
    result = await Runner.run(
        ...
        run_config=RunConfig(sandbox=SandboxRunConfig(client=client, options=NightonaSandboxClientOptions())),
    )
    await client.close()
...
**Note:** `resolve_exposed_port` requires the port to be predeclared in the client options, e.g. `NightonaSandboxClientOptions(exposed_ports=(8080,))`. Without this, the call raises `ExposedPortUnavailableError`.
...
## Quick Reference: NightonaSandboxClientOptions
