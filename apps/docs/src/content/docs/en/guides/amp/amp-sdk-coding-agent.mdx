...
🔧 create_file /home/nightona/index.html
Successfully created file /home/nightona/index.html
🔧 create_file /home/nightona/start.sh
...
First, clone the nightona [repository](https://github.com/Amartuvshins0404/nightona.git) and navigate to the example directory:
...
4. Sends a Nightona-aware system prompt as the first user message (preview URL pattern + instruction to write server startup command to `/home/nightona/start.sh`).
...
A Nightona-aware system prompt is sent as the first user message. It instructs the agent to use the preview URL pattern and to write the server start command into `/home/nightona/start.sh` (instead of executing directly in Amp), then provide the preview URL:
...
The readline loop waits for user input, sends it to the agent, and displays the streamed response. If Amp produced `/home/nightona/start.sh`, the script is then launched via Nightona's session command API so long-running/background server startup does not hang Amp turns.
