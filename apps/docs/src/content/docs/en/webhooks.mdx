---
title: Webhooks
description: Connect your applications to Nightona events in real-time with webhooks for automation, monitoring, and integrations.
---

Webhooks are HTTP callbacks that Nightona sends to your specified endpoints when specific events occur.
Think of them as "reverse API calls" - instead of your application asking Nightona for updates, Nightona
...
- **Integration**: connect Nightona with existing tools like Slack, Discord, or custom applications
- **Audit & compliance**: maintain detailed logs of all important changes

:::note
Webhooks are available to organization admins and members with appropriate permissions. If you don't see **Webhooks** in [Nightona Dashboard ↗](https://app.daytona.io/dashboard), contact [support@daytona.io](mailto:support@daytona.io) to enable webhooks for your organization.
:::

## Accessing webhooks

Nightona provides a webhook management interface to access and manage webhook endpoints.
...
Nightona provides a webhook management interface to create webhook endpoints.
...
Nightona provides a webhook management interface to edit webhook endpoints.
...
Nightona provides a webhook management interface to delete webhook endpoints.
...
Nightona sends webhooks for lifecycle events across your infrastructure resources. You can subscribe to specific event types or receive all events and filter them in your application.
