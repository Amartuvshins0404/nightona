...
Declarative Builder provides a powerful, code-first approach to defining dependencies for Nightona Sandboxes. Instead of importing images from a container registry, you can programmatically define them using the Nightona SDK.
...
Nightona provides an option to create declarative images on-the-fly when creating sandboxes. This is ideal for iterating quickly without creating separate snapshots.
...
declarative_image = Nightona::Image
  .debian_slim('3.12')
  .pip_install(['requests', 'pytest'])
  .workdir('/home/nightona')
...
# Create a new Sandbox with the declarative image and stream the build logs
sandbox = nightona.create(
  Nightona::CreateSandboxFromImageParams.new(image: declarative_image),
...
// Define a declarative image with python packages
version := "3.12"
declarativeImage := nightona.DebianSlim(&version).
...
Sandbox sandbox = nightona.create(params, 0L, System.out::println);
```
...
Nightona provides an option to [create pre-built snapshots](/docs/snapshots#create-snapshots) that can be reused across multiple sandboxes.

The snapshot remains visible in the [Nightona Dashboard ↗](https://app.daytona.io/dashboard/snapshots) and is permanently cached, ensuring instant availability without rebuilding.
...
# Create a new Sandbox using the pre-built Snapshot
sandbox = nightona.create(
  Nightona::CreateSandboxFromSnapshotParams.new(snapshot: snapshot_name)
)
```
...
Sandbox sandbox = nightona.create(snapshotParams);
```
...
Nightona provides an option to define images programmatically using the Nightona SDK. You can specify base images, install packages, add files, set environment variables, and more.

For a complete API reference and method signatures, see the [Python](/docs/python-sdk/common/image), [TypeScript](/docs/typescript-sdk/image), [Ruby](/docs/ruby-sdk/image), [Go](/docs/go-sdk/nightona#type-DockerImage), and [Java](/docs/java-sdk/image) SDK references.

### Base image selection

Nightona provides an option to select base images. The following snippets demonstrate how to select and configure base images:
...
```ruby
# Create an image from a base
image = Nightona::Image.base('python:3.12-slim-bookworm')
...
```go
// Create an image from a base
image := nightona.Base("python:3.12-slim-bookworm")
...
Nightona provides an option to install packages and dependencies to your image.
...
```ruby
# Add pip packages
image = Nightona::Image.debian_slim('3.12').pip_install(['requests', 'pandas'])

# Install from requirements.txt
image = Nightona::Image.debian_slim('3.12').pip_install_from_requirements('requirements.txt')

# Install from pyproject.toml (with optional dependencies)
image = Nightona::Image.debian_slim('3.12').pip_install_from_pyproject('pyproject.toml',
...
// Add pip packages
version := "3.12"
image := nightona.DebianSlim(&version).PipInstall([]string{"requests", "pandas"})
...
Nightona provides an option to add files and directories to your image.
...
```typescript
// Add a local file
const image = Image.debianSlim('3.12').addLocalFile('package.json', '/home/nightona/package.json')

// Add a local directory
const image = Image.debianSlim('3.12').addLocalDir('src', '/home/nightona/src')
```
...
```ruby
# Add a local file
image = Nightona::Image.debian_slim('3.12').add_local_file('package.json', '/home/nightona/package.json')

# Add a local directory
image = Nightona::Image.debian_slim('3.12').add_local_dir('src', '/home/nightona/src')
```
...
// Add a local file
version := "3.12"
image := nightona.DebianSlim(&version).AddLocalFile("package.json", "/home/nightona/package.json")

// Add a local directory
image := nightona.DebianSlim(&version).AddLocalDir("src", "/home/nightona/src")
```
...
Nightona provides an option to configure environment variables and working directories.
...
```typescript
// Set environment variables
const image = Image.debianSlim('3.12').env({ PROJECT_ROOT: '/home/nightona' })

// Set working directory
const image = Image.debianSlim('3.12').workdir('/home/nightona')
```
...
```ruby
# Set environment variables
image = Nightona::Image.debian_slim('3.12').env({ 'PROJECT_ROOT' => '/home/nightona' })

# Set working directory
image = Nightona::Image.debian_slim('3.12').workdir('/home/nightona')
```
...
// Set environment variables
version := "3.12"
image := nightona.DebianSlim(&version).Env("PROJECT_ROOT", "/home/nightona")

// Set working directory
image := nightona.DebianSlim(&version).Workdir("/home/nightona")
```
...
// Set environment variables
Image image = Image.debianSlim("3.12")
    .env(java.util.Map.of("PROJECT_ROOT", "/home/nightona"));

// Set working directory
image = Image.debianSlim("3.12").workdir("/home/nightona");
```
...
Nightona provides an option to execute commands during build and configure container startup behavior.
...
```ruby
# Run shell commands during build
image = Nightona::Image.debian_slim('3.12').run_commands(
  'apt-get update && apt-get install -y git',
  'groupadd -r nightona && useradd -r -g nightona -m nightona',
  'mkdir -p /home/nightona/workspace'
)
...
# Set entrypoint
image = Nightona::Image.debian_slim('3.12').entrypoint(['/bin/bash'])

# Set default command
image = Nightona::Image.debian_slim('3.12').cmd(['/bin/bash'])
```
...
image := nightona.DebianSlim(&version).
  ...
  Run("groupadd -r nightona && useradd -r -g nightona -m nightona").
  Run("mkdir -p /home/nightona/workspace")

// Set entrypoint
image := nightona.DebianSlim(&version).Entrypoint([]string{"/bin/bash"})

// Set default command
image := nightona.DebianSlim(&version).Cmd([]string{"/bin/bash"})
```
...
Nightona provides an option to integrate existing Dockerfiles or add custom Dockerfile commands.
...
```ruby
# Add custom Dockerfile commands
image = Nightona::Image.debian_slim('3.12').dockerfile_commands(['RUN echo "Hello, world!"'])

# Use an existing Dockerfile
image = Nightona::Image.from_dockerfile('Dockerfile')

# Extend an existing Dockerfile
image = Nightona::Image.from_dockerfile('app/Dockerfile').pip_install(['numpy'])
```
...
image := nightona.FromDockerfile(string(content))
...
Nightona provides an option to install OS-level packages during the image build. Use this pattern when your sandbox needs CLI tools or system libraries that are not available through `pip`.
...
Nightona provides an option to define a non-root user for application workloads. Run all installation steps as `root` first, then create the user, fix ownership of the working directory, and switch to the new user with the `USER` directive. Subsequent commands and the sandbox runtime then operate without root privileges.
...
image = (
  ...
  .run_commands(
    "groupadd -r nightona && useradd -r -g nightona -m -d /home/nightona nightona",
    "chown -R nightona:nightona /home/nightona",
  )
  .workdir("/home/nightona")
...
const image = Image.debianSlim('3.12')
  ...
  .workdir('/home/nightona')
  .dockerfileCommands(['USER nightona'])
```
...
image := nightona.DebianSlim(&version).
  ...
  Run("groupadd -r nightona && useradd -r -g nightona -m -d /home/nightona nightona").
  Run("chown -R nightona:nightona /home/nightona").
  Workdir("/home/nightona").
  User("nightona")
```
...
Nightona provides an option to combine multiple language runtimes in a single image. The following pattern adds Node.js 20 to a Python base image by installing it from the NodeSource repository. The same approach works for adding Go, Ruby, Java, or any other runtime that distributes a Linux installer.
