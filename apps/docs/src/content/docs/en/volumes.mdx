...
- **Must be absolute paths**: mount paths must start with `/` (e.g., `/home/nightona/volume`)
...
const mountDir = '/home/nightona/volume'
...
public class App {
    public static void main(String[] args) {
        try (Nightona nightona = new Nightona()) {
            ...
            String mountDir = "/home/nightona/volume";
...
```shell
nightona volume create my-awesome-volume
nightona create --volume my-awesome-volume:/home/nightona/volume
```
...
curl 'https://app.daytona.io/api/sandbox' \
  ...
  "volumes": [
    {
      "volumeId": "<VOLUME_ID>",
      "mountPath": "/home/nightona/volume",
      "subpath": "users/alice"
    }
  ]
}'
...
sandbox.fs.upload_file(b"Hello from Nightona volume!", "/home/nightona/volume/example.txt")
...
await sandbox.fs.uploadFile(
  ...
  '/home/nightona/volume/example.txt'
)
...
```ruby
# Write to a file in the mounted volume using the Sandbox file system API
sandbox.fs.upload_file('Hello from Nightona volume!', '/home/nightona/volume/example.txt')
...
err := sandbox.FileSystem.UploadFile(context.Background(), []byte("Hello from Nightona volume!"), "/home/nightona/volume/example.txt")
if err != nil {
    log.Fatal(err)
}
...
sandbox = nightona.create(CreateSandboxFromSnapshotParams(
    language="python",
    volumes=[
        VolumeMount(volume_id=shared_assets.id, mount_path="/home/nightona/assets"),
        VolumeMount(volume_id=logs.id, mount_path="/home/nightona/logs"),
    ],
))
```
...
const sandbox = await nightona.create({
  language: 'typescript',
  volumes: [
    { volumeId: sharedAssets.id, mountPath: '/home/nightona/assets' },
    { volumeId: logs.id, mountPath: '/home/nightona/logs' },
  ],
})
```
...
params = Nightona::CreateSandboxFromSnapshotParams.new(
  language: Nightona::CodeLanguage::PYTHON,
  volumes: [
    NightonaApiClient::SandboxVolume.new(volume_id: shared_assets.id, mount_path: '/home/nightona/assets'),
    NightonaApiClient::SandboxVolume.new(volume_id: logs.id, mount_path: '/home/nightona/logs')
  ]
)
sandbox = nightona.create(params)
```
...
assetsMount.setMountPath("/home/nightona/assets");

VolumeMount logsMount = new VolumeMount();
logsMount.setVolumeId(logs.getId());
logsMount.setMountPath("/home/nightona/logs");
...
const mountDir = '/home/nightona/data'
