...
Nightona provides SSH access to your sandboxes using token-based authentication. This allows you to connect from local terminals, IDEs, and development tools without installing additional software.

## Access from Dashboard

Create an SSH access token directly from the [Nightona Dashboard ↗](https://app.daytona.io/dashboard/sandboxes).
...
Nightona generates a token and displays it in the modal. Copy the token and use it to connect to your sandbox.

## Access via CLI

Nightona provides a CLI command to create an SSH access token for a sandbox:

```shell  
nightona create
```

When you create a sandbox, Nightona displays the SSH command automatically in the output:

```text
Sandbox '<sandboxId>' created successfully
Connect via SSH:         nightona ssh <sandboxId>
Open the Web Terminal:   https://22222-<sandboxId>.proxy.nightona.work
```

To SSH into an existing sandbox, use the following command:

```bash
nightona ssh <sandbox> --expires 60
```
...
nightona = Nightona()
sandbox = nightona.get("sandbox-abc123")
...
const sandbox = await nightona.get('sandbox-abc123')
