---
title: Architecture
description: Nightona platform architecture overview.
---

import ArchitectureDiagram from '@components/ArchitectureDiagram.astro'

Nightona provides **full composable computers** — [sandboxes](/docs/en/sandboxes) — for AI agents. Nightona platform is organized into multiple plane components, each serving a specific purpose:

- [Interface plane](#interface-plane) provides client interfaces for interacting with Nightona
...
The interface plane provides client interfaces for users and agents to interact with Nightona. The following components are part of the interface plane and available to all users and agents:
...
The control plane is the central coordination layer of the Nightona platform. It receives all client requests, manages the full sandbox lifecycle, schedules sandboxes onto runners, and continuously reconciles states across the infrastructure. The control plane includes the following components:
...
To interact with sandboxes from the API, see the [API](/docs/en/tools/api) and [Toolbox API](/docs/en/tools/api#nightona-toolbox) references.
...
Runners are compute nodes that power Nightona's compute plane, providing the underlying infrastructure for running sandbox workloads. Each runner polls the control plane API for jobs and executes sandbox operations: creating, starting, stopping, destroying, resizing, and backing up sandboxes. Runners interact with S3-compatible object storage for snapshot and volume data, and with the internal snapshot registry.
...
The sandbox daemon is a code execution agent that runs inside each sandbox. It exposes the [Toolbox API](/docs/en/tools/api#nightona-toolbox), providing direct access to the sandbox environment: file system and Git operations, process and code execution, computer use, log streaming, and terminal sessions.
