...
Bring Your Own Compute (BYOC) enables you to use your own runner machines to run sandbox workloads. Runners are machines that power Nightona's compute plane and provide the underlying infrastructure. Each runner is responsible for:

- **Workload execution**: running sandbox workloads
- **Resource management**: allocating and monitoring CPU, memory, and disk resources
- **Health reporting**: providing metrics and health status to the Nightona control plane
- **Network connectivity**: managing sandbox networking, proxy connections, and SSH access

Runners in [shared](/docs/en/regions#shared-regions) and [dedicated](/docs/en/regions#dedicated-regions) regions are fully managed by Nightona — from provisioning and maintenance to monitoring and scaling. For custom regions, you bring your own runner machines and are responsible for their management and operation.

Nightona provides two sets of runner management endpoints: [custom runners](#custom-runners) (`/runners`) and [admin operations](#admin-operations) (`/admin/runners`). While both support creating, listing, updating, and deleting runners, they serve different purposes and offer different levels of control.

Custom runner endpoints are scoped to your [organization](/docs/en/organizations) and support the `X-Nightona-Organization-ID` header. Admin endpoints operate across the entire platform and do not require an organization header.
...
Nightona provides methods to create a custom region using the [Nightona Dashboard ↗](https://app.daytona.io/dashboard/regions) or programmatically using the [API](/docs/en/tools/api#nightona/tag/organizations/POST/regions).

1. Navigate to [Nightona Dashboard ↗](https://app.daytona.io/dashboard/regions)
...
```json
{
  ...
  "snapshotManagerUsername": "nightona",
  "snapshotManagerPassword": "generated-password"
}
```

For more information, see the [API](/docs/en/tools/api#nightona/tag/organizations) reference:

> [**createRegion (API)**](/docs/en/tools/api#nightona/tag/organizations/POST/regions)

### Custom region credentials

When you create a custom region, Nightona will provide credentials for any optional services you configure:

- An API key that should be used by your proxy service to authenticate with Nightona
- An API key that should be used by your SSH gateway service to authenticate with Nightona
- Basic authentication credentials that Nightona uses to access your snapshot manager service
...
Nightona provides methods to list all available regions for your organization using the [Nightona Dashboard ↗](https://app.daytona.io/dashboard/regions) or programmatically using the [API](/docs/en/tools/api#nightona/tag/organizations/GET/regions).
...
> [**listAvailableRegions (API)**](/docs/en/tools/api#nightona/tag/organizations/GET/regions)

### Get a custom region

Nightona provides methods to get the details of a specific region using the [Nightona Dashboard ↗](https://app.daytona.io/dashboard/regions) or programmatically using the [API](/docs/en/tools/api#nightona/tag/organizations/GET/regions/{id}).
...
> [**getRegionById (API)**](/docs/en/tools/api#nightona/tag/organizations/GET/regions/{id})

### Update a custom region

Nightona provides methods to update the configuration of an existing custom region using the [Nightona Dashboard ↗](https://app.daytona.io/dashboard/regions) or programmatically using the [API](/docs/en/tools/api#nightona/tag/organizations/PATCH/regions/{id}).
...
> [**updateRegion (API)**](/docs/en/tools/api#nightona/tag/organizations/PATCH/regions/{id})

### Delete a custom region

Nightona provides methods to delete a custom region using the [Nightona Dashboard ↗](https://app.daytona.io/dashboard/regions) or programmatically using the [API](/docs/en/tools/api#nightona/tag/organizations/DELETE/regions/{id}). Regions that have runners assigned to them cannot be deleted.
...
> [**deleteRegion (API)**](/docs/en/tools/api#nightona/tag/organizations/DELETE/regions/{id})

#### Regenerate proxy API key

Nightona provides an API to regenerate the proxy API key for a custom region.
...
> [**regenerateProxyApiKey (API)**](/docs/en/tools/api#nightona/tag/organizations/POST/regions/{id}/regenerate-proxy-api-key)

#### Regenerate SSH gateway API key

Nightona provides an API to regenerate the SSH gateway API key for a custom region.
...
> [**regenerateSshGatewayApiKey (API)**](/docs/en/tools/api#nightona/tag/organizations/POST/regions/{id}/regenerate-ssh-gateway-api-key)

#### Regenerate snapshot manager credentials

Nightona provides an API to regenerate the snapshot manager credentials for a custom region.
...
> [**regenerateSnapshotManagerCredentials (API)**](/docs/en/tools/api#nightona/tag/organizations/POST/regions/{id}/regenerate-snapshot-manager-credentials)
...
Nightona provides methods to create a custom runner using the [Nightona Dashboard ↗](https://app.daytona.io/dashboard/runners) or programmatically using the [API](/docs/en/tools/api#nightona/tag/runners/POST/runners).

1. Navigate to [Nightona Dashboard ↗](https://app.daytona.io/dashboard/runners)
...
Upon creating a runner, you will be presented with a secure token that you can use to authenticate the runner with the Nightona control plane. The response includes the runner ID and the API key token:
...
For more information, see the [API](/docs/en/tools/api#nightona/tag/runners) reference:

> [**createRunner (API)**](/docs/en/tools/api#nightona/tag/runners/POST/runners)

### List custom runners

Nightona provides methods to list all runners in your organization using the [Nightona Dashboard ↗](https://app.daytona.io/dashboard/runners) or programmatically using the [API](/docs/en/tools/api#nightona/tag/runners/GET/runners).
...
> [**listRunners (API)**](/docs/en/tools/api#nightona/tag/runners/GET/runners)

### Get a custom runner

Nightona provides methods to get the details of a specific runner using the [Nightona Dashboard ↗](https://app.daytona.io/dashboard/runners) or programmatically using the [API](/docs/en/tools/api#nightona/tag/runners/GET/runners/{id}).
...
> [**getRunnerById (API)**](/docs/en/tools/api#nightona/tag/runners/GET/runners/{id})

### Update runner scheduling

Nightona provides methods to update the scheduling status of a runner using the [Nightona Dashboard ↗](https://app.daytona.io/dashboard/runners) or programmatically using the [API](/docs/en/tools/api#nightona/tag/runners/PATCH/runners/{id}/scheduling). This allows you to mark a runner as unschedulable, preventing new sandboxes from being assigned to it.
...
> [**updateRunnerScheduling (API)**](/docs/en/tools/api#nightona/tag/runners/PATCH/runners/{id}/scheduling)

### Delete a custom runner

Nightona provides methods to delete a custom runner using the [Nightona Dashboard ↗](https://app.daytona.io/dashboard/runners) or programmatically using the [API](/docs/en/tools/api#nightona/tag/runners/DELETE/runners/{id}).
...
> [**deleteRunner (API)**](/docs/en/tools/api#nightona/tag/runners/DELETE/runners/{id})

## Admin operations

Admin runners operate across the entire platform. They do not require the `X-Nightona-Organization-ID` header and return `RunnerFull` objects with additional `apiKey` and `regionType` fields.
...
- The API key used to authenticate the runner with the Nightona control plane
...
Nightona provides an admin API to create a runner with full configuration, including resource capacity and network settings. Unlike the custom runner endpoint, you must provide your own `apiKey` and specify the `apiVersion` (`0` or `2`).
...
For more information, see the [API](/docs/en/tools/api#nightona/tag/admin) reference:

> [**adminCreateRunner (API)**](/docs/en/tools/api#nightona/tag/admin/POST/admin/runners)

### List runners

Nightona provides an admin API to list all runners with full details, including resource usage metrics. You can optionally filter runners by region.
...
> [**adminListRunners (API)**](/docs/en/tools/api#nightona/tag/admin/GET/admin/runners)

### Get runner

Nightona provides an admin API to get full runner details, including resource usage and allocation metrics.
...
> [**adminGetRunnerById (API)**](/docs/en/tools/api#nightona/tag/admin/GET/admin/runners/{id})

### Update runner scheduling

Nightona provides an admin API to update the scheduling status of a runner.
...
> [**adminUpdateRunnerScheduling (API)**](/docs/en/tools/api#nightona/tag/admin/PATCH/admin/runners/{id}/scheduling)

### Delete runner

Nightona provides an admin API to delete a runner.
...
> [**adminDeleteRunner (API)**](/docs/en/tools/api#nightona/tag/admin/DELETE/admin/runners/{id})

## Runner deployment

After registering a custom runner and obtaining its secure token, install and configure the Nightona runner application on your infrastructure. Nightona provides official [Terraform modules](#terraform) and [Helm charts](#helm-charts) for automated runner deployment.

### Terraform

Nightona provides a Terraform module for deploying runners on AWS EC2 with automated installation and configuration. The module handles instance provisioning, security configuration, and runner registration with the Nightona platform.
...
- Nightona runner .deb package hosted at an accessible URL

#### Basic example

```hcl
module "nightona_runner" {
  source = "./runner"
  ...
  # Nightona Configuration
  api_url   = "https://nightona.example.com/api"
...
module "nightona_runner" {
  ...
  # Nightona Configuration
  api_url   = "https://api.nightona.example.com"
...
| **`api_url`**                       | Nightona API URL                                | string       | -             | yes          |
| **`api_key`**                       | Nightona API key                                | string       | -             | yes          |
| **`region_id`**                     | Nightona region ID                              | string       | -             | yes          |
| **`name_prefix`**                   | Prefix for resource names                      | string       | "nightona"     | no           |
| **`runner_name`**                   | Name for the runner (used in API registration) | string       | null          | no           |
| **`runner_version`**                | Nightona runner version                         | string       | "0.125.0-rc1" | no           |
...
| Name                | Description                        |
| ------------------- | ---------------------------------- |
| runner_id           | Nightona runner ID                  |
| runner_name         | Nightona runner name                |
...
# Check runner status
sudo systemctl status nightona-runner

# View logs
sudo journalctl -u nightona-runner -f
```
...
# Service logs
sudo journalctl -u nightona-runner -n 100 --no-pager

# Check configuration
sudo cat /etc/nightona/runner.env
```
...
ls -la /opt/nightona/runner

# Check binary permissions
file /opt/nightona/runner
```

### Helm charts

Nightona provides a Helm chart for deploying custom regions and their supporting infrastructure on Kubernetes. The `nightona-region` chart deploys a proxy service, optional snapshot manager, and a registration job, allowing you to run Nightona sandboxes within your own Kubernetes cluster.
...
- Downloads the Nightona runner binary
- Installs Docker if missing
- Registers the runner with the Nightona API
...
```bash
sudo tail -f /var/log/nightona-runner.log
```

Stop service:

```bash
sudo systemctl stop nightona-runner
```
...
| **`LOG_FILE_PATH`**         | Path to runner log file      | `/var/log/nightona-runner.log`                 |
...
| **`AWS_DEFAULT_BUCKET`**    | AWS S3 bucket name           | `nightona`                                     |
...
| **`SERVER_URL`**            | Nightona API URL              | User-provided                                 |
