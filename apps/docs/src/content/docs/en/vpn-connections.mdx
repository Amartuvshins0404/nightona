...
VPN connections are a way to connect your Nightona sandboxes to private networks. By establishing a VPN connection, your sandbox can access network resources using private IP addresses and can be accessed by other devices on the same VPN network.
...
Nightona supports the following VPN network providers:
...
:::note
For connecting to a VPN network, you need to [create or access your Nightona sandbox](/docs/en/sandboxes), **have access to your VPN network provider credentials**, and be on [**Tier 3** or higher](/docs/en/limits#resources).
:::

## Tailscale

Nightona provides multiple ways to connect to a Nightona sandbox with a Tailscale network:
...
When you connect a Nightona sandbox to a Tailscale network, the sandbox becomes part of your private Tailscale network, allowing you to access resources that are available within the network and enabling other devices on the network to access the sandbox.
...
# Configuration
NIGHTONA_API_KEY = "YOUR_API_KEY" # Replace with your API key

# Initialize the Nightona client
config = NightonaConfig(api_key=NIGHTONA_API_KEY)
nightona = Nightona(config)

def setup_tailscale_vpn_interactive():
    """
    Connect a Nightona sandbox to a Tailscale network using the Python SDK.
...
// Configuration
const NIGHTONA_API_KEY = 'YOUR_API_KEY' // Replace with your API key

// Initialize the Nightona client
const nightona = new Nightona({
  apiKey: NIGHTONA_API_KEY,
})
...
async function setupTailscaleVpnInteractive(): Promise<void> {
  /**
   * Connect a Nightona sandbox to a Tailscale network using the TypeScript SDK.
   * Uses interactive login via browser URL (no auth key required).
   */
...
Using an auth key provides a non-interactive way to connect your Nightona sandbox to Tailscale, making it suitable for automated scripts, CI/CD pipelines, or any scenario where manual browser interaction is not available.
...
Copy the auth key from the generated script and use it to connect your Nightona sandbox to Tailscale:
...
For working directly in the terminal or for more control over the Tailscale connection process, you can set up Tailscale manually through the Nightona [web terminal](/docs/en/web-terminal) or [SSH](/docs/en/ssh-access).
...
1. Navigate to your sandbox [web terminal](/docs/en/web-terminal) in [Nightona Dashboard ↗](https://app.daytona.io/), or [access it via SSH](/docs/en/ssh-access)
...
This begins the Tailscale installation process and initializes the Tailscale CLI inside the sandbox. Nightona requires the Tailscale daemon to be running in the background to connect the sandbox to the Tailscale network.
...
You've now successfully connected your Nightona sandbox to your Tailscale network. The sandbox should appear in your [Tailscale dashboard](https://login.tailscale.com/admin/machines).

## OpenVPN

Nightona provides multiple ways to connect to a Nightona sandbox with an OpenVPN network:

- [OpenVPN client configuration](#openvpn-client-configuration)
- [Connect with web terminal](#openvpn-web-terminal)

OpenVPN uses a client-server model where your Nightona sandbox acts as a client connecting to an OpenVPN server. This approach is suitable for connecting to existing corporate VPNs, accessing resources behind firewalls, or integrating with infrastructure that already uses OpenVPN.

:::note
Connecting a Nightona sandbox to OpenVPN network requires a [client configuration file](#openvpn-client-configuration-file).
:::
...
def setup_openvpn(ovpn_config: str):
    """
    Connect a Nightona sandbox to an OpenVPN network using the Python SDK.
    """
    # Create the sandbox
    print("Creating sandbox...")
    sandbox = nightona.create()
    ...
    sandbox.fs.upload_file(ovpn_config.encode(), "/home/nightona/client.ovpn")
    print("Configuration written to /home/nightona/client.ovpn")

    # Step 3: Start OpenVPN in background
    print("\nStarting OpenVPN tunnel...")
    sandbox.process.exec(
        "nohup sudo openvpn /home/nightona/client.ovpn > /tmp/openvpn.log 2>&1 &",
        timeout=10
    )
...
async function setupOpenvpn(ovpnConfig: string): Promise<void> {
  /**
   * Connect a Nightona sandbox to an OpenVPN network using the TypeScript SDK.
   */
  ...
  await sandbox.process.executeCommand(
    `cat << 'OVPNEOF' > /home/nightona/client.ovpn
${ovpnConfig}
OVPNEOF`,
    undefined,
    undefined,
  ...
  console.log("Configuration written to /home/nightona/client.ovpn");
...
Nightona provides a [web terminal](/docs/en/web-terminal) for interacting with your sandboxes, allowing you to install OpenVPN and connect to your OpenVPN network.

1. Navigate to your sandbox [web terminal](/docs/en/web-terminal) in [Nightona Dashboard ↗](https://app.daytona.io/) by clicking on the Terminal icon `>_`, or [access it via SSH](/docs/en/ssh-access)
...
3. Create the OpenVPN [client configuration file](#openvpn-client-configuration-file) for your Nightona sandbox
...
Nightona requires the OpenVPN tunnel to be running in the background to connect the sandbox to the OpenVPN network. The recommended approach is to run it in a detached tmux (or similar) session:
...
You've now successfully connected your Nightona sandbox to your OpenVPN network.

## Netbird

Nightona provides multiple ways to connect to a Nightona sandbox with a Netbird network:

- [Connect with setup key](#netbird-setup-key)
- [Connect with browser login](#netbird-browser-login)

When you connect a Nightona sandbox to a Netbird network, the sandbox becomes part of your private Netbird network, allowing you to access resources that are available within the network and enabling other devices on the network to access the sandbox.
...
Using a Netbird setup key provides a non-interactive way to connect your Nightona sandbox to Netbird, making it suitable for automated scripts, CI/CD pipelines, or any scenario where manual browser interaction is not available.
...
def setup_netbird():
    """
    Connect a Nightona sandbox to a NetBird network using the Python SDK.
...
// Configuration
const NIGHTONA_API_KEY = "YOUR_API_KEY";
...
async function setupNetbird(): Promise<void> {
  /**
   * Connect a Nightona sandbox to a NetBird network using the TypeScript SDK.
   * Uses the official NetBird install script and a Setup Key for non-interactive auth.
   * See: https://docs.netbird.io/manage/peers/access-infrastructure/setup-keys-add-servers-to-network
   */
...
The browser login method initiates an interactive authentication flow where Netbird generates a unique login URL that you visit in your web browser to authenticate the Nightona sandbox.
