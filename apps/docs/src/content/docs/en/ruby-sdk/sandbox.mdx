...
Sandbox class for Nightona SDK.
...
- `NightonaApiClient:SandboxState` - The state of the sandbox
...
- `NightonaApiClient:SandboxDesiredState` - The desired state of the sandbox
...
- `Array\<NightonaApiClient:SandboxVolume\>, nil` - Volumes attached to the sandbox.
Not returned by list results; call #refresh on each item to populate.
...
- `NightonaApiClient:BuildInfo, nil` - Build information for the sandbox if it was
...
- `NightonaApiClient:SshAccessDto`
...
- `NightonaApiClient:PortPreviewUrl`
...
- `NightonaApiClient:SignedPortPreviewUrl` - The signed preview URL response object
...
- `resources` _Nightona:Resources_ - New resource configuration (cpu, memory, disk only)
- `timeout` _Numeric_ - Maximum wait time in seconds (defaults to 60 s)
...
- `language_id` _Symbol_ - The language server type (e.g., Nightona::LspServer::Language::PYTHON)
...
- `NightonaApiClient:SshAccessValidationDto`
...
- `Nightona:Sandbox` - The forked Sandbox
